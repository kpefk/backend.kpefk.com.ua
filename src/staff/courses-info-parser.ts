...
function tryParseCsv(fragment: string): ParsedUpgrade | null {
	...
	const docIdx = parts.findIndex(p =>
		DOC_TYPES.some(d => p.toLowerCase().startsWith(d))
	)
	...
	if (!isDateRange && !isSingleDate && !isUnspecified) {
		...
		const potentialCert = afterDoc
			.replace(/^(свідоцтво|сертифікат|диплом)\s*/i, '')
			.trim()
	...
	const courseNameParts =
		hoursIdx >= 0
			? remainingParts.slice(0, hoursIdx)
			: remainingParts.slice(0, -1)

	// Пропускаємо поле з датою видачі (якщо є між назвою та годинами)
	const coursePartsFiltered = courseNameParts.filter(
		p => !parseDate(p.trim())
	)
	const courseName = coursePartsFiltered
		.join(', ')
		.replace(/[«»]/g, '')
		.trim()
...
function tryParseFreeText(fragment: string): ParsedUpgrade | null {
	...
	const orgMatch = fragment.match(
		/^([^,«]+?)(?:\s*,|\s+(?:сертифікат|свідоцтво|диплом|дистанційно|онлайн|курс))/i
	)
	const orgName = orgMatch ? orgMatch[1].trim() : null

	// Номер сертифіката: послідовність букв+цифр що не є датою
	const certMatch = fragment.match(
		/\b([A-ZА-ЯІЇЄ]{1,10}\d{5,}|\d{4,}\/\d+)\b/i
	)
