...
export class StudentController {
	...
	@ApiOperation({
		summary: 'Переглянути згенерований email без створення акаунту'
	})
	@ApiParam({ name: 'id', description: 'UUID студента' })
	@ApiResponse({
		status: 200,
		schema: { example: { email: 'kovalenko.i1503.d1.25@kpefk.com.ua' } }
	})
	@ApiResponse({ status: 404, description: 'Студента не знайдено' })
	...
	@ApiOperation({
		summary: 'Створити корпоративний акаунт Google Workspace для студента'
	})
	@ApiParam({ name: 'id', description: 'UUID студента' })
	@ApiResponse({
		status: 200,
		description: 'Акаунт створено або вже існував',
		schema: {
			example: {
				email: 'kovalenko.i1503.d1.25@kpefk.com.ua',
				created: true
			}
		}
	})
