...
export class GroupsService {
	...
	public async getIncomingTransfers(groupId: string) {
		return this.prisma.studentGroupHistory.findMany({
			...
			include: {
				student: {
					select: { id: true, personFIO: true, groupName: true }
				},
	...
	public async syncFromStudents(
		...
		if (changedStudents.length === 0) {
			const archival = await this.syncGroupArchival(new Date())
			return { created, total: uniqueNames.length, moves: 0, ...archival }
		}
		...
		const reasonByStudent = this.detectChangeReason(
			changedStudents,
			oldGroupSnapshot
		)
		...
		for (const entry of changedStudents) {
			...
			const reason =
				reasonByStudent.get(entry.id) ?? GroupChangeReason.EDEBO_SYNC
		...
		const archival = await this.syncGroupArchival(new Date())
		return { created, total: uniqueNames.length, moves, ...archival }
	}

	// ── Архівація груп ────────────────────────────────────────────

	/**
	 * Архівує групи, у яких не лишилось активних студентів (усі завершили навчання чи
	 * відраховані), і відкріплює від них куратора. Академвідпустка лишає групу активною.
	 * Якщо в архівній групі знову зʼявляються активні студенти — знімає архів (куратор — вручну).
	 */
	private async syncGroupArchival(
		now: Date
	): Promise<{ archived: number; reactivated: number }> {
		// «Живі» студенти: ще не завершили навчання (educationDateEnd порожній або в майбутньому)
		// і не відраховані.
		const keepAlive = await this.prisma.student.groupBy({
			by: ['groupId'],
			where: {
				groupId: { not: null },
		...
		for (const r of keepAlive) {
			if (r.groupId !== null) aliveByGroup.set(r.groupId, r._count._all)
		}

		const withStudents = await this.prisma.student.groupBy({
			by: ['groupId'],
			where: { groupId: { not: null } },
			_count: { _all: true }
		})
		const totalByGroup = new Map<string, number>()
		for (const r of withStudents) {
			if (r.groupId !== null) totalByGroup.set(r.groupId, r._count._all)
		}
		...
		for (const g of groups) {
			const total = totalByGroup.get(g.id) ?? 0
			const alive = aliveByGroup.get(g.id) ?? 0
			if (total > 0 && alive === 0 && g.archivedAt === null) {
				toArchive.push(g.id)
			} else if (alive > 0 && g.archivedAt !== null) {
				toReactivate.push(g.id)
			}
		}

		// Архів: ставимо archivedAt і відкріплюємо куратора.
		if (toArchive.length > 0) {
			await this.prisma.group.updateMany({
				where: { id: { in: toArchive } },
				data: { archivedAt: now, curatorId: null }
			})
		}
	...
	private detectChangeReason(
		...
		for (const s of changedStudents) {
			...
			destinations.set(
				s.toGroupName,
				(destinations.get(s.toGroupName) ?? 0) + 1
			)
		}
		...
		for (const student of changedStudents) {
			...
			const totalInOldGroup =
				oldGroupSnapshot.get(student.fromGroupName) ?? 0
