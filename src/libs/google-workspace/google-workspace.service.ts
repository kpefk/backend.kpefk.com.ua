...
export class GoogleWorkspaceService {
	...
	public constructor(private readonly configService: ConfigService) {
		const auth = new google.auth.GoogleAuth({
			...
			clientOptions: {
				subject: this.configService.getOrThrow<string>(
	...
	public async provisionAccount(
		...
		try {
			...
			this.logger.error(
				`Failed to provision ${email}: ${err?.message ?? String(error)}`
			)
	...
	private parseName(student: StudentEmailInput): {
		lastName: string
		firstInitial: string
	} {
		const source =
	...
	private parseFullName(student: StudentEmailInput): {
		lastName: string
		firstName: string
	} {
		const source =
	...
	private parseBirthday(birthday: Date | null): {
		day: string
		month: string
	} {
