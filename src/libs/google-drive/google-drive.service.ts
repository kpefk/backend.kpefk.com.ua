...
export class GoogleDriveService {
	...
	public constructor(private readonly configService: ConfigService) {
		...
		this.rootFolderId = this.configService.getOrThrow<string>(
	...
	public async uploadPassport(
		...
		try {
			...
			throw new InternalServerErrorException(
				'Failed to upload passport to Google Drive.'
			)
		}
	}
	...
	public async deleteFile(googleFileId: string): Promise<void> {
		try {
			...
			this.logger.error(
				`Delete failed for ${googleFileId}: ${err?.message ?? String(error)}`
			)
			throw new InternalServerErrorException(
	...
	public async streamFile(
		...
		try {
			...
			throw new InternalServerErrorException(
				'Failed to stream file from Google Drive.'
			)
		}
	}
	...
	public async getFileMetadata(googleFileId: string) {
		try {
			...
			this.logger.error(
				`Failed to get metadata for ${googleFileId}`,
				error
			)
			throw new InternalServerErrorException(
