...
export class DiplomaService {
	...
	public async getGradeSheet(batchId: string) {
		...
		const groups = [...byGroup.entries()]
			...
			.map(([groupName, ds]) => {
				const reference =
					ds.find(d => d.components.length > 0)?.components ?? []
				...
				const rows = ds.map(d => ({
					...
					grades: Object.fromEntries(
						d.components.map(c => [
							c.orderIndex,
							{ componentId: c.id, grade: c.grade }
						])
					)
				}))
				return {
					groupName,
					specialityName:
						ds.find(d => d.specialityName)?.specialityName ?? null,
	...
	public async applyTemplateToBatch(
		...
		const diplomas = await this.prisma.diploma.findMany({
			where: {
				batchId,
				...(groupName ? { studyGroupName: groupName } : {})
			},
			select: { id: true }
		})
	...
	public async update(id: string, dto: UpdateDiplomaDto) {
		...
		await this.prisma.$transaction(async tx => {
			...
			if (templateChanged) {
				await tx.diplomaComponent.deleteMany({
					where: { diplomaId: id }
				})
				if (dto.templateId) {
					const components =
						await tx.diplomaTemplateComponent.findMany({
	...
	private totalEcts(components: ComponentRow[]): number {
		return components.reduce(
			(sum, c) => sum + (c.ects === null ? 0 : Number(c.ects)),
			0
		)
	}
	...
	private computeHonors(components: ComponentRow[]): boolean {
		...
		if (scaled.some(c => c.grade === DiplomaGrade.SATISFACTORY))
			return false

		const excellent = scaled.filter(
			c => c.grade === DiplomaGrade.EXCELLENT
		).length
		if (excellent / scaled.length < HONORS_EXCELLENT_RATIO) return false

		const attestations = components.filter(
			c => c.type === 'ATTESTATION' && c.grade
		)
		if (attestations.some(c => c.grade !== DiplomaGrade.EXCELLENT))
			return false
