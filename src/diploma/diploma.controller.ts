...
} from './diploma-generator.service'
...
export class DiplomaController {
	...
	@ApiOperation({
		summary:
			'Призначити шаблон усім дипломам партії та застосувати знімок компонентів'
	})
	...
	public applyTemplateToBatch(
		...
		return this.diplomaService.applyTemplateToBatch(
			id,
			dto.templateId ?? null,
			dto.groupName
		)
	...
	@ApiOperation({
		summary: "Видалити партію дипломів (і всі пов'язані дипломи)"
	})
	@Delete('batches/:id')
	...
	@ApiOperation({
		summary:
			'Підтягнути з ЄДЕБО документи про освіту (підстава для вступу) для партії'
	})
	...
	@ApiOperation({
		summary: 'Зведена відомість партії: всі студенти × ОК + оцінки'
	})
	...
	@ApiOperation({
		summary: 'Акредитація ЄДЕБО для спеціальності (пошук за кодом/назвою)'
	})
	...
	@ApiOperation({
		summary:
			'Синхронізувати дані акредитації з ЄДЕБО для всіх шаблонів дипломів'
	})
	...
	public async generateBulk(
		...
		const { buffer, filename } = await this.generatorService.generateBulk(
			batchId,
			doc
		)
	...
	@ApiOperation({
		summary: 'Один диплом (повна картка зі зведеною відомістю)'
	})
	@Get(':id')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Оновити диплом (шаблон, тема роботи, відзнака, статус)'
	})
	@Patch(':id')
	@HttpCode(HttpStatus.OK)
	...
	public assignTemplate(
		...
		return this.diplomaService.assignTemplate(
			id,
			dto.templateId,
			dto.applyToGroup ?? false
		)
	}
	...
	public async generate(
		...
		const { buffer, filename } = await this.generatorService.generateOne(
			id,
			doc
		)
