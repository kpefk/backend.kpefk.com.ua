...
export type DiplomaRow = Prisma.DiplomaGetPayload<{
	include: typeof DIPLOMA_INCLUDE
}>
...
export class DiplomaGeneratorService {
	...
	public async generateBulk(
		...
		if (diplomas.length === 0)
			throw new NotFoundException('У партії немає дипломів.')
	...
	private renderDiploma(
		...
		const who = `${diploma.lastNameUk}_${diploma.firstNameUk}`.replace(
			/\s+/g,
			'_'
		)
	...
	private templateBuffer(
		...
		const bytes =
			kind === 'diploma'
				? diploma.template?.diplomaDocx
				: diploma.template?.addendumDocx
	...
	public buildData(d: DiplomaRow): Record<string, unknown> {
		...
		return {
			...
			graduateYear: d.graduateDate
				? String(d.graduateDate.getUTCFullYear())
				: '',
			...
			components: d.components.map((c, i) => ({
				...
				gradeUk: c.grade ? GRADE_LABELS[c.grade].uk : '',
				gradeEn: c.grade ? GRADE_LABELS[c.grade].en : ''
			})),
	 ...
	 */
	private studyPeriod(d: DiplomaRow, templatePeriod: string | null): string {
