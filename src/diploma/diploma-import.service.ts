...
import { type ParsedDiplomaOrder, parseDiplomaXml } from './diploma-xml.parser'
...
export class DiplomaImportService {
	...
	public preview(file: Express.Multer.File) {
		...
		for (const o of orders) {
			...
			if (existing) existing.count++
				...
				bySpecialty.set(name, {
					code: o.specialtyCode ?? '',
					name,
					count: 1
				})
		}
		return {
			count: orders.length,
			specialties: [...bySpecialty.values()].sort((a, b) =>
	...
	public async commit(
		...
		const resolveTemplate = (order: ParsedDiplomaOrder): string | null => {
			...
			const matches = activeTemplates.filter(t => {
				...
				return (
					...
					tn !== '' &&
					(tn === orderName ||
						tn.includes(orderName) ||
						orderName.includes(tn))
				)
			})
		...
		const matchStudent = (o: ParsedDiplomaOrder) =>
			(o.edeboPersonCode
				? studentByCode.get(o.edeboPersonCode)
				: undefined) ??
			(o.personEducationId != null
				? studentByEduId.get(o.personEducationId)
				: undefined) ??
			null
		...
		this.logger.log(
			`Diploma batch ${batch.id}: imported ${orders.length} diplomas`
		)
	...
	private parse(file: Express.Multer.File): ParsedDiplomaOrder[] {
		...
		if (orders.length === 0) {
			throw new BadRequestException(
				'У XML не знайдено жодного <Document>.'
			)
		}
		return orders
	}
	...
	private mapOrder(
		...
		Prisma.DiplomaUncheckedCreateInput,
		'batchId' | 'templateId' | 'components'
	> {
		return {
			lastNameUk: o.lastNameUk,
			firstNameUk: o.firstNameUk,
