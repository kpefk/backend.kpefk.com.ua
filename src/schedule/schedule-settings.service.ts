...
export class ScheduleSettingsService {
	...
	public async update(
		...
		const updated = await this.prisma.scheduleSettings.update({
			...
			data: {
				...
				maxPairsPartTime:
					dto.maxPairsPartTime === undefined
						? undefined
						: dto.maxPairsPartTime,
	 ...
	 */
	public async maxPairsForForm(
		form: EducationForm | null
	): Promise<number | null> {
		const s = await this.get()
		if (form === EducationForm.FULL_TIME || form === null)
			return s.maxPairsFullTime
