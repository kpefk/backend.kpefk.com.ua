...
export const SUBJECT_TEACHER_INCLUDE = {
	...
	lessonAssignments: {
		include: { overrideTeacher: { select: TEACHER_MINI } }
	}
...
export type TeacherMini = Prisma.TeacherGetPayload<{
	select: typeof TEACHER_MINI
}>
...
export type EntryRow = Prisma.ScheduleEntryGetPayload<{
...
export function timeOverlaps(
	...
	if (a.dayOfWeek !== b.dayOfWeek || a.slotNumber !== b.slotNumber)
		return false
...
export function mapEntry(
	...
	for (const other of allPeriodEntries) {
		...
		if (row.classroomId && other.classroomId === row.classroomId) {
			conflicts.push(
				`Аудиторія зайнята: ${row.classroom?.number ?? ''}`.trim()
			)
		}
	...
	if (ctx.maxPairsPerDay != null) {
		...
		if (rank > ctx.maxPairsPerDay) {
			conflicts.push(
				`Перевищено ліміт пар на день (${ctx.maxPairsPerDay})`
			)
		}
	}
