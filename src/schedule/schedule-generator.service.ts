...
const TYPE_HOUR_FIELDS = [
	{
		lessonType: 'LECTURE',
		hoursField: 'lectureHours',
		weeklyField: 'weeklyLectureHours'
	},
	{
		lessonType: 'PRACTICE',
		hoursField: 'practicalHours',
		weeklyField: 'weeklyPracticalHours'
	},
...
export class ScheduleGeneratorService {
	...
	public async generate(
		...
		const workingCurriculumId =
			await this.scheduleService.resolveWorkingCurriculumId(
				groupId,
				academicYear
			)
		...
		const teachingWeeks = await this.resolveTeachingWeeks(
			wc.versionId,
			semesterNumber
		)
		...
		if (classrooms.length === 0) {
			warnings.push(
				'У системі немає аудиторій — заняття створено без аудиторій.'
			)
		}
		...
		for (const unit of units) {
			...
			placement.assignments.forEach((a, i) => {
				...
				if (sub.teacherId)
					this.addKeys(teacherBusy, sub.teacherId, keys)
				if (a.classroomId)
					this.addKeys(classroomBusy, a.classroomId, keys)
	...
	public async generateAll(
		...
		const eligible = await this.scheduleService.getEligibleGroups(
			dto.academicYear
		)
		const targets = eligible.filter(
			g =>
				g.hasWorkingCurriculum &&
				g.semesterNumbers.length > positionIndex
		)
		...
		for (const g of targets) {
			...
			try {
				...
				this.logger.warn(
					`generateAll: група ${g.groupName} — ${message}`
				)
				results.push({
					groupId: g.groupId,
					groupName: g.groupName,
	...
	private buildUnits(
		...
		for (const wt of workingTerms) {
			...
			for (const {
				...
				const pairsPerCycle = this.pairsPerCycle(
					weekly,
					hours,
					teachingWeeks
				)
	...
	private placeUnit(
		...
		const parityOptions: WeekParity[] = unit.isHalf
			? ['ODD', 'EVEN']
			: ['EVERY']
		const passes: ('pack' | 'any')[] = unit.isHalf
			? ['pack', 'any']
			: ['any']
				...
				for (let slot = 1; slot <= maxPairs; slot++) {
					for (const parity of parityOptions) {
						...
						if (pass === 'pack') {
							...
							if (!groupBusy.has(`${day}:${slot}:${opposite}`))
								continue
						}
	...
	private assignClassrooms(
		...
		for (let i = 0; i < count; i++) {
			const free = classroomIds.find(
				cid =>
					!chosen.includes(cid) &&
					this.isFree(classroomBusy.get(cid), keys)
			)
			if (!free) return null
			chosen.push(free)
		}
