...
export class ScheduleService {
	...
	public async getEligibleGroups(
		...
		const wcByVersion = new Map(workingCurricula.map(w => [w.versionId, w]))
		...
		return result.sort((x, y) =>
			x.groupName.localeCompare(y.groupName, 'uk')
		)
	}
	...
	public async massReplace(dto: MassReplaceDto): Promise<ScheduleDto> {
		...
		if (Object.keys(data).length === 0) {
			throw new BadRequestException(
				'Нічого замінювати: не задано викладача чи аудиторію.'
			)
		}
