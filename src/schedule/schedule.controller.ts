...
export class ScheduleController {
	...
	public getByTeacher(
		...
		return this.scheduleService.getByTeacher(
			teacherId,
			academicYear,
			semesterNumber
		)
	...
	public getByClassroom(
		...
		return this.scheduleService.getByClassroom(
			classroomId,
			academicYear,
			semesterNumber
		)
	...
	@ApiOperation({
		summary: 'Редагувати заняття (перенести/змінити викладача/аудиторію)'
	})
	...
	public async updateEntry(
		...
		await this.audit.record(userId, 'ENTRY_UPDATE', result.id, {
			entryId: id
		})
		return result
	}
	...
	@ApiResponse({
		status: 200,
		description: 'ScheduleDto (перерахований з конфліктами)'
	})
	@Authorization(...DISPATCHER_ROLES)
	@Post('entries/swap')
	@HttpCode(HttpStatus.OK)
	...
	public async deleteEntry(
		...
		await this.audit.record(userId, 'ENTRY_DELETE', result.id, {
			entryId: id
		})
		return result
	}
	...
	@ApiOperation({
		summary: 'Призначити/прибрати день виховної години для групи'
	})
	...
	@ApiOperation({
		summary: 'Скопіювати розклад як шаблон у цільову групу/семестр'
	})
	...
	public async removeSubstitution(
		...
		await this.audit.record(userId, 'SUBSTITUTION_DELETE', result.id, {
			subId: id
		})
		return result
	}
