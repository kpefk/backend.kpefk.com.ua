...
export class ScheduleExportService {
	...
	public async toIcs(
		...
		const startMonday = this.semesterStartMonday(
			academicYear,
			semesterNumber
		)
		const weeks = await this.semesterWeeks(
		...
		for (const e of schedule.entries) {
			...
			if (e.subgroupNumber !== null)
				summaryParts.push(`${e.subgroupNumber} підгр.`)

			const descParts: string[] = []
			if (e.teacher)
				descParts.push(`Викладач: ${teacherShort(e.teacher)}`)
			...
			if (e.classroom)
				lines.push(
					`LOCATION:${this.escape(`ауд. ${e.classroom.number}`)}`
				)
	...
	private recurrence(
		...
		const start = this.addDays(
			startMonday,
			firstWeekOffset * 7 + (dayOfWeek - 1)
		)
		const count =
			parity === 'EVERY'
				? weeks
				: Math.ceil((weeks - firstWeekOffset) / 2)
