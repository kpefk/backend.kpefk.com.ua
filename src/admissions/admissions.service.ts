...
export class AdmissionsService {
	...
	public async listYears(): Promise<AdmissionYearDto[]> {
		...
		for (const c of campaigns) {
			const [offersCount, applicationsCount] = await Promise.all([
				this.prisma.admissionOffer.count({
					where: { admissionYear: c.admissionYear }
				}),
	...
	public async getOverview(year: number): Promise<AdmissionOverviewDto> {
		...
		const likes = apps.map(a => this.toAppLike(a))
		...
		const averageKonkurs =
			konkurs.length > 0
				? Math.round(
						(konkurs.reduce((s, v) => s + v, 0) / konkurs.length) *
							100
					) / 100
				: null
	...
	public async listOffers(year: number): Promise<AdmissionOfferRowDto[]> {
		...
		const offers = await this.prisma.admissionOffer.findMany({
			...
			orderBy: [
				{ specialityCode: 'asc' },
				{ universitySpecialitiesName: 'asc' }
			],
			include: {
	...
	public async bySpeciality(
		year: number
	): Promise<AdmissionSpecialityRowDto[]> {
		await this.requireCampaign(year)
		const offers = await this.prisma.admissionOffer.findMany({
			where: { admissionYear: year },
			select: {
	...
	public async konkursDistribution(
		...
		return {
			admissionYear: year,
			buckets: bucketKonkurs(values, bucketSize)
		}
	}
	...
	private applyApplicationFilters(
		...
		return rows.filter(a => {
			...
			if (
				f.claim === 'budget' &&
				!(a.isClaimForBudget && !a.isClaimForContract)
			)
				return false
			if (
				f.claim === 'contract' &&
				!(a.isClaimForContract && !a.isClaimForBudget)
			)
				return false
			if (
				f.claim === 'both' &&
			...
			if (f.requirements === 'yes' && !a.isOriginalDocumentsAdded)
				return false
			if (f.requirements === 'no' && a.isOriginalDocumentsAdded)
				return false
			return true
		})
	}
	...
	private async fetchApplicationRows(
		year: number
	): Promise<AdmissionApplicationRowDto[]> {
		const campaign = await this.requireCampaign(year)
		...
		const apps = await this.prisma.admissionApplication.findMany({
			...
			include: {
				offer: {
					select: { specialityName: true, educationFormName: true }
				}
			}
		})
		return apps.map(a => ({
			...
			konkursValue:
				a.konkursValue !== null ? Number(a.konkursValue) : null,
			...
			entryEduDocDateGet: a.entryEduDocDateGet
				? a.entryEduDocDateGet.toISOString()
				: null
		}))
	}
	...
	private buildApplicationsXlsx(
		...
		const claimLabel = (a: AdmissionApplicationRowDto): string => {
			if (a.isClaimForBudget && a.isClaimForContract)
				return 'Бюджет + Контракт'
	...
	public async getSettings(year: number): Promise<AdmissionSettingsDto> {
		...
		const siteByOffer = new Map(
			siteCounts.map(r => [r.offerId, r._count._all])
		)
		...
		return {
			...
			registrationDescryptionDefault:
	...
	public async updateOfferSettings(
		...
		if (!offer)
			throw new NotFoundException('Конкурсну пропозицію не знайдено.')

		const updated = await this.prisma.admissionOffer.update({
			where: { id: offer.id },
			data: {
				...(dto.caseSuffix !== undefined
					? { caseSuffix: dto.caseSuffix.trim() || null }
					: {}),
				...(dto.registrationDescryption !== undefined
					? {
							registrationDescryption:
								dto.registrationDescryption.trim() || null
						}
					: {})
			},
			select: {
	...
	private async requireCampaign(year: number) {
		...
		if (!campaign)
			throw new NotFoundException('Кампанію за цей рік не знайдено.')
		return campaign
	}
	...
	private toAppLike(a: {
		...
		return {
			konkursValue:
				a.konkursValue !== null && a.konkursValue !== undefined
					? Number(a.konkursValue)
					: null,
			isClaimForBudget: a.isClaimForBudget,
			isClaimForContract: a.isClaimForContract,
