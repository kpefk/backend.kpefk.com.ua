...
export class AdmissionsController {
	...
	@ApiOperation({
		summary: 'Зведення за рік (KPI + воронка + бюджет/контракт)'
	})
	@Get('overview')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Розподіл заяв за спеціальностями (розбиття по ОПП)'
	})
	@Get('by-speciality')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Операційний список заяв із ПД (лише активна кампанія)'
	})
	...
	@ApiOperation({
		summary: 'Експорт заяв вступників у .xlsx (з урахуванням фільтрів)'
	})
	@Authorization(UserRole.ADMINISTRATOR)
	...
	@ApiOperation({
		summary: 'Архівувати рік — вичистити ПД, лишити звітні поля'
	})
	@Authorization(UserRole.ADMINISTRATOR)
	@Post(':year/archive')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Налаштування кампанії + КП (суфікси справ, реєстрація)'
	})
	@Authorization(UserRole.ADMINISTRATOR)
	@Get('settings')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Оновити налаштування кампанії (авто-реєстрація + ресинк)'
	})
	@Authorization(UserRole.ADMINISTRATOR)
	@Patch('settings')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Оновити налаштування КП (суфікс справи + текст реєстрації)'
	})
	@Authorization(UserRole.ADMINISTRATOR)
	@Patch('offers/:universitySpecialitiesId/settings')
	@HttpCode(HttpStatus.OK)
	public updateOfferSettings(
		@Param('universitySpecialitiesId', ParseIntPipe)
		universitySpecialitiesId: number,
		@Query() query: AdmissionYearQueryDto,
		@Body() dto: UpdateOfferSettingsDto
	) {
		return this.service.updateOfferSettings(
			query.year,
	...
	@ApiOperation({
		summary: 'Ручний запуск авто-реєстрації (1 → 5 + номер справи)'
	})
	@Authorization(UserRole.ADMINISTRATOR)
	@Post('auto-register')
	@HttpCode(HttpStatus.OK)
	...
	@ApiOperation({
		summary: 'Підтягнути документи про освіту заочних вступників з ЄДЕБО'
	})
	@Authorization(UserRole.ADMINISTRATOR)
