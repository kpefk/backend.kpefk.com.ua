...
export class AdmissionSyncService {
	...
	public async autoPollTick(): Promise<void> {
		...
		if (
			!campaign ||
			campaign.status !== AdmissionCampaignStatus.ACTIVE ||
			!campaign.pollEnabled
		) {
			return
		}
		...
		const inWindow =
			hour >= campaign.pollWindowStartHour &&
			hour < campaign.pollWindowEndHour
		const intervalMs =
			(inWindow
				? campaign.pollIntervalActiveSec
				: campaign.pollIntervalOffHoursSec) * 1000
		const elapsed = campaign.lastAutoPollAt
			? now.getTime() - campaign.lastAutoPollAt.getTime()
			: Infinity
	...
	public async autoRegisterPending(
		...
		if (campaign.status === AdmissionCampaignStatus.ARCHIVED) {
			throw new BadRequestException(
				'Архівний рік заморожено — авто-реєстрація недоступна.'
			)
		}
		...
		const pending = await this.prisma.admissionApplication.findMany({
			where: {
				...
				OR: assignCaseNumber
					? [
							...
							{
								statusTypeId: String(REGISTERED_STATUS_TYPE_ID),
								personalCode: null
							}
						]
					: [{ statusTypeId: SITE_STATUS_ID }]
			...
			orderBy: [
				{ dateCreate: { sort: 'asc', nulls: 'last' } },
				{ personRequestId: 'asc' }
			]
		})
		...
		for (const app of pending) {
			const alreadyRegistered =
				app.statusTypeId === String(REGISTERED_STATUS_TYPE_ID)
			...
			if (assignCaseNumber) {
				...
				nextNum =
					nextNumByOffer.get(app.offerId) ??
					(await this.computeNextCaseNumber(app.offerId))
				personalCode = `${String(nextNum).padStart(2, '0')}-${suffix}`
			}

			const registrationDescryption =
				app.offer.registrationDescryption ??
				campaign.registrationDescryptionDefault ??
				''

			try {
				if (dryRun) {
					const numPart = personalCode
						? `, № справи=${personalCode}`
						: ' (без номера)'
					this.logger.log(
					...
					if (personalCode) {
						await this.entrance.personRequestUpdate({
							...
							isAnotherBudgetAllowed:
								app.isAnotherBudgetAllowed ?? false,
							isSimiliarSpeciality:
								app.isSimiliarSpeciality ?? false,
							isOriginalDocumentsAdded:
								app.isOriginalDocumentsAdded ?? false,
							informationOriginalDocumentLocation:
								app.informationOriginalDocumentLocation ??
								false,
							isConfirmedContract:
								app.isConfirmedContract ?? false,
							...(app.enrollPriority !== null
								? { enrollPriority: app.enrollPriority }
								: {}),
							...(app.foreignTypeId !== null
								? { foreignTypeId: app.foreignTypeId }
								: {}),
					...
					await this.prisma.auditLog.create({
						data: {
							...
							metadata: {
								...
								statusFrom: alreadyRegistered
									? REGISTERED_STATUS_TYPE_ID
									: Number(SITE_STATUS_ID),
				...
				failed++
				this.logger.error(
					`Авто-реєстрація не вдалась для заяви ${app.personRequestId}`,
					err
				)
			}
		}
	 ...
	 */
	public async fetchEntryEducationDocs(
		admissionYear: number
	): Promise<EntryEducationDocResult> {
		const pending = await this.prisma.admissionApplication.findMany({
			where: {
				admissionYear,
		...
		const byGroup = new Map<
			string,
			{ appIds: string[]; personCodeU: string; base: string | null }
		>()
		...
		for (const [, g] of byGroup) {
			let docs: EdboEntrantDoc[] | null | undefined =
				docsCacheByPerson.get(g.personCodeU)
			...
			if (docs === undefined) {
				try {
					...
					this.logger.warn(
						`Документи особи ${g.personCodeU} недоступні: ${String(err)}`
					)
	...
	public async syncYear(admissionYear: number): Promise<AdmissionSyncResult> {
		if (this.isSyncing) {
			throw new BadRequestException(
				'Синхронізація вже виконується. Зачекайте завершення.'
			)
		}
		...
		if (existing?.status === AdmissionCampaignStatus.ARCHIVED) {
			throw new BadRequestException(
				'Архівний рік заморожено — синхронізація недоступна.'
			)
		}
		...
		try {
			await this.prisma.admissionCampaign.upsert({
				...
				create: {
					admissionYear,
					status: AdmissionCampaignStatus.ACTIVE
				},
			...
			for (const offer of offers) {
				if (
					offer.universitySpecialitiesId === null ||
					offer.universitySpecialitiesId === undefined
				) {
					continue
				}
			...
			try {
				...
				this.logger.error(
					`Fetch entry education docs failed for ${admissionYear}`,
					err
				)
			}
	...
	public async archiveYear(
		...
		const result = await this.prisma.$transaction(async tx => {
			...
			await tx.admissionCampaign.update({
				where: { admissionYear },
				data: {
					status: AdmissionCampaignStatus.ARCHIVED,
		...
		this.logger.log(
			`Admission year ${admissionYear} archived; PII purged for ${result} applications`
		)
	...
	private async syncApplications(
		...
		let count = 0
		for (let pageNo = 0; ; pageNo++) {
			const page = await this.fetchApplicationsPage(
				universitySpecialitiesId,
				pageNo
			)
			for (const app of page) {
				...
				if (
					app.personRequestId === null ||
					app.personRequestId === undefined
				)
					continue
				const personRequestId = String(app.personRequestId)
				const data = this.mapApplication(
					app,
					offerId,
					admissionYear,
					syncedAt,
	...
	private async fetchApplicationsPage(
		...
		if (this.list2VariantIndex !== null) {
			...
			return this.edbo.post<PersonRequestList2ResponseDto[]>(
		...
		for (let i = 0; i < LIST2_BODY_VARIANTS.length; i++) {
			...
			try {
				const result = await this.edbo.post<
					PersonRequestList2ResponseDto[]
				>(
					PERSON_REQUEST_LIST2_PATH,
					variant.build(
						universitySpecialitiesId,
				...
				this.logger.log(
					`personRequest/list2 body-форма визначена: «${variant.label}»`
				)
				return result
			} catch (err) {
				lastErr = err
	...
	private mapApplication(
		...
		return {
			...
			statusTypeId:
				a.personRequestStatusTypeId !== null &&
				a.personRequestStatusTypeId !== undefined
			...
			informationOriginalDocumentLocation:
				a.informationOriginalDocumentLocation ?? null,
			budgetRecommendationTypeId: a.budgetRecommendationTypeId ?? null,
			contractRecommendationTypeId:
				a.contractRecommendationTypeId ?? null,
