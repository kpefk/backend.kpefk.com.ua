...
export function computeGradeStats(grades: number[], scale: GradeScale): GradeStats {
  const total = grades.length

  if (total === 0) {
    const ranges = scale === GradeScale.TWELVE_POINT
      ? ['10-12', '7-9', '4-6', '1-3']
      : ['5', '4', '3', '2']
    return {
  ...
  if (scale === GradeScale.TWELVE_POINT) {
    ...
    return {
      ranges: [
        { label: '10-12', count: r1012, percent: formatPercent(r1012, total) },
        { label: '7-9', count: r79, percent: formatPercent(r79, total) },
        { label: '4-6', count: r46, percent: formatPercent(r46, total) },
