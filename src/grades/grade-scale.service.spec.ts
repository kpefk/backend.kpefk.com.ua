...
describe('GradeScaleService', () => {
	...
	describe('toNationalGrade — CREDIT throws', () => {
		it('throws for CREDIT control form', () => {
			expect(() =>
				service.toNationalGrade(
					null,
					GradeScale.TWELVE_POINT,
					TermControlForm.CREDIT
				)
	...
	describe('validateCreditGrade', () => {
		it('allows PASSED', () => {
			expect(() =>
				service.validateCreditGrade(NationalGrade.PASSED)
			).not.toThrow()
		})

		it('allows NOT_PASSED', () => {
			expect(() =>
				service.validateCreditGrade(NationalGrade.NOT_PASSED)
			).not.toThrow()
		})

		it('rejects EXCELLENT', () => {
			expect(() =>
				service.validateCreditGrade(NationalGrade.EXCELLENT)
			).toThrow(BadRequestException)
		})
	})
})
