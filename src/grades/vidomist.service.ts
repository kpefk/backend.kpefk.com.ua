...
export class VidomistService {
  ...
  public async generate(
    ...
    const data = await this.gatherData(componentTermId, groupId, academicYear)
  ...
  private async gatherData(
    ...
    const teacher = await this.prisma.teacherLoadSubjectAssignment.findFirst({
    ...
    const rows: VidomistStudentRow[] = students.map((s, idx) => {
      ...
      const gradeText = controlForm === TermControlForm.CREDIT
        ? formatCreditText(g.nationalGrade)
        : g.finalGrade !== null
          ? formatGradeText(g.finalGrade, gradeScale, g.nationalGrade)
          : ''
    ...
    const specialty = term.component.section.version.curriculum.program.specialty
  ...
  private buildHeader(
    ...
    const paragraphs: Paragraph[] = [
      new Paragraph({
        ...
        spacing: { after: 40 },
        children: [
          new TextRun({ text: 'Міністерство освіти і науки України', size: 24 }),
        ],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER,
  ...
  private buildMetadata(
    ...
    const roman = ROMAN_NUMERALS[data.semesterNumber] ?? String(data.semesterNumber)
    ...
    return [
      new Paragraph({
        spacing: { after: 60 },
        children: [
          new TextRun({ text: `Група ${data.groupName}  `, bold: true, size: 24 }),
          new TextRun({
      ...
      new Paragraph({
        spacing: { after: 60 },
        children: [
          new TextRun({ text: 'Освітній компонент ', bold: true, size: 24 }),
      ...
      new Paragraph({
        spacing: { after: 60 },
        children: [
          new TextRun({ text: `Семестр ${roman}  `, bold: true, size: 24 }),
      ...
      new Paragraph({
        spacing: { after: 240 },
        children: [
          new TextRun({ text: 'Дата проведення «', bold: true, size: 24 }),
  ...
  private buildTable(rows: VidomistStudentRow[]): Table {
    const headerRow = new TableRow({
      ...
      children: [
        ...
        this.headerCell(COL_NAME, 'Прізвище, ім’я по-батькові студента'),
    ...
    const dataRows = rows.map((row) =>
      new TableRow({
        children: [
          this.dataCell(COL_NUM, `${row.index}.`, AlignmentType.CENTER),
          this.dataCell(COL_NAME, row.fullName, AlignmentType.LEFT),
          this.dataCell(COL_GRADE, row.gradeText, AlignmentType.CENTER),
  ...
  private buildFooter(data: {
    ...
    return [
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `К-сть «${stats.ranges[0].label}»  ${formatCount(stats.ranges[0].count, stats.ranges[0].percent)}\t` +
      ...
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `К-сть «${stats.ranges[1].label}»  ${formatCount(stats.ranges[1].count, stats.ranges[1].percent)}\t` +
  ...
  private buildCreditFooter(rows: VidomistStudentRow[]): Paragraph[] {
    const passed = rows.filter((r) => r.nationalGrade === NationalGrade.PASSED).length
    const notPassed = rows.filter((r) => r.nationalGrade === NationalGrade.NOT_PASSED).length
    const total = rows.filter((r) => r.gradeText !== '').length

    const pctPassed = total > 0 ? `${((passed / total) * 100).toFixed(1)}%` : '—'
    const pctNotPassed = total > 0 && notPassed > 0 ? `${((notPassed / total) * 100).toFixed(1)}%` : '—'
    ...
    return [
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `К-сть «зараховано»  ${passed}   «${pctPassed}»\t` +
