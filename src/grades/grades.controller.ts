...
import { BulkGradeEntryDto, CreateSemesterGradeDto } from './dto/create-semester-grade.dto'
import {
...
import { GradesService, type GradesActor } from './grades.service'
...
export class GradesController {
  ...
  public recordGradesBulk(
    ...
    return this.gradesService.recordGradesBulk(dto, this.actor(userId, role))
  }
  ...
  @ApiOperation({ summary: 'Залікова книжка студента (студент — лише власну)' })
  ...
  public getStudentTranscript(
    ...
    return this.gradesService.getStudentTranscript(studentId, query, this.actor(userId, role))
  }
  ...
  @ApiOperation({ summary: 'Мої дисципліни для оцінювання (тільки для TEACHER)' })
  @Get('my-disciplines')
  @HttpCode(HttpStatus.OK)
  ...
  @ApiOperation({ summary: 'Вагова схема формули підказки підсумкової оцінки' })
  @Get('weight-settings')
  @HttpCode(HttpStatus.OK)
  ...
  public updateWeightSettings(
    ...
    if (!isElevatedGrades(role)) {
      throw new ForbiddenException('Недостатньо прав для зміни вагової схеми')
    }
