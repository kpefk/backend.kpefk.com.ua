...
export class GradesService {
	...
	public async recordGrade(
		dto: CreateSemesterGradeDto,
		actor: GradesActor
		...
		if (newAttempt > MAX_ATTEMPTS) {
			throw new BadRequestException(
				`Максимальна кількість спроб — ${MAX_ATTEMPTS}`
			)
		}
	...
	public async recordGradesBulk(
		...
		for (const g of dto.grades) {
			if (!validStudentIds.has(g.studentId)) {
				throw new BadRequestException(
					`Студент ${g.studentId} не належить до групи`
				)
			}
		}
	...
	public async getGradeSheet(
		...
		const attendanceSummary =
			await this.attendanceSummaryService.getAttendanceSummary(
				componentTermId,
				query.groupId,
				query.academicYear,
				term.semesterNumber
	...
	public async getScaleInfo(
		componentTermId: string
	): Promise<GradeScaleInfoDto> {
		const term =
			await this.prisma.curriculumComponentTerm.findUniqueOrThrow({
				where: { id: componentTermId },
				select: {
	...
	private resolveGradeValues(
		...
		if (controlForm === TermControlForm.CREDIT) {
			if (!nationalGradeOverride) {
				throw new BadRequestException(
					'Для заліку необхідно вказати nationalGradeOverride (PASSED/NOT_PASSED)'
				)
			}
		...
		const nationalGrade = this.scaleService.toNationalGrade(
			finalGrade,
			gradeScale,
			controlForm
		)
	...
	// ── Recognized grades (перезарахування / мобільність) ──────────────────────

	/**
	 * Матеріалізує перезараховану/визнану оцінку в заліковій книжці.
	 * Викликається з `confirm()` модулів визнання (уже гейтяться manage-ролями),
	 * тому НЕ перевіряє закріплення викладача (на відміну від `recordGrade`).
	 * Приймає транзакційний клієнт — щоб генерація всіх оцінок акту була атомарною.
	 * Заміна: наявна ACTIVE-оцінка → SUPERSEDED, нова → ACTIVE (як при перездачі).
	 */
	public async recordRecognizedGrade(
		...
		const term = await tx.curriculumComponentTerm.findUniqueOrThrow({
			where: { id: input.curriculumComponentTermId },
			select: {
				controlForm: true,
				component: {
					select: { section: { select: { sectionType: true } } }
		...
		const { finalGrade, nationalGrade } = this.resolveGradeValues(
			input.finalGrade,
			input.nationalGradeOverride,
			gradeScale,
			controlForm
		)

		const currentMax = await tx.semesterGrade.aggregate({
			where: {
				studentId: input.studentId,
				curriculumComponentTermId: input.curriculumComponentTermId
			},
			_max: { attempt: true }
		})
		const prevAttempt = currentMax._max.attempt ?? 0
		...
		if (prevAttempt > 0) {
			const prevActive = await tx.semesterGrade.findFirst({
				where: {
					studentId: input.studentId,
					curriculumComponentTermId: input.curriculumComponentTermId,
					status: SemesterGradeStatus.ACTIVE
			...
			previousActiveId = prevActive?.id ?? null
			await tx.semesterGrade.updateMany({
				where: {
					studentId: input.studentId,
					curriculumComponentTermId: input.curriculumComponentTermId,
		...
		const created = await tx.semesterGrade.create({
			data: {
				...
				status: SemesterGradeStatus.ACTIVE,
				origin: input.origin,
				previousAttemptId: previousActiveId,
	...
	/**
	 * Скасовує перезараховану оцінку: видаляє її й, якщо вона заміняла попередню
	 * (previousAttemptId), повертає ту попередню в ACTIVE. Використовується в `revert()`.
	 */
	public async deleteRecognizedGrade(
		tx: Prisma.TransactionClient,
		gradeId: string
	): Promise<void> {
		const grade = await tx.semesterGrade.findUnique({
			where: { id: gradeId },
			select: { previousAttemptId: true }
		})
		if (!grade) return
		await tx.semesterGrade.delete({ where: { id: gradeId } })
		if (grade.previousAttemptId) {
			await tx.semesterGrade.update({
				where: { id: grade.previousAttemptId },
				data: { status: SemesterGradeStatus.ACTIVE }
			})
		}
	}
	...
	private async assertCanGrade(
		...
		if (actor.role !== UserRole.TEACHER) {
			throw new ForbiddenException(
				'Тільки викладач або керівництво може виставляти оцінки'
			)
		}
		...
		if (!teacher)
			throw new ForbiddenException('Профіль викладача не знайдено')
