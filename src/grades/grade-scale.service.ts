...
export class GradeScaleService {
	...
	public toNationalGrade(
		...
		if (grade === null) {
			throw new BadRequestException(
				"Числова оцінка обов'язкова для іспиту/диф. заліку"
			)
		}
	...
	public validateGrade(
		...
		if (controlForm === TermControlForm.CREDIT) {
			if (grade !== null) {
				throw new BadRequestException(
					'Залік не має числової оцінки — тільки Зараховано/Не зараховано'
				)
			}
			return
		}
	...
	public validateCreditGrade(nationalGrade: NationalGrade): void {
		if (
			nationalGrade !== NationalGrade.PASSED &&
			nationalGrade !== NationalGrade.NOT_PASSED
		) {
			throw new BadRequestException(
				'Для заліку допустимі лише PASSED або NOT_PASSED'
			)
		}
	}
