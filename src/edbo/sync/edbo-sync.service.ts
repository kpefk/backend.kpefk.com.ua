...
import { SYNC_KEYS, SyncStateService } from './sync-state.service'
...
export class EdboSyncService {
	...
	public async scheduledSync(): Promise<void> {
		...
		if (students.status === 'fulfilled') {
			...
			this.logger.log(
				`Students: created=${r.created} updated=${r.updated} skipped=${r.skipped} total=${r.total}`
			)
		} else {
		...
		if (staff.status === 'fulfilled') {
			const r = staff.value
			this.logger.log(
				`Staff: created=${r.created} updated=${r.updated} skipped=${r.skipped} total=${r.total}`
			)
		} else {
	...
	public async scheduledUniversitySync(): Promise<void> {
		...
		try {
			...
			this.logger.log(
				`University weekly sync done: ${r.name ?? r.universityId}`
			)
		} catch (error) {
	...
	public async syncUniversity(): Promise<UniversitySyncResult> {
		...
		this.logger.log(
			`University sync done: ${saved.name ?? this.universityId}`
		)
		return {
			universityId: this.universityId,
			name: saved.name,
	...
	public async syncStudents(fromDate?: string): Promise<SyncResult> {
		...
		while (true) {
			...
			for (const record of records) {
				...
				// Core-поля (курс/група/статус) оновлюємо завжди — ЄДЕБО змінює їх (курс за
				// календарем, випуск/переведення) без зміни modifyDate. Дорогий fetch документів
				// робимо лише для нових або змінених записів.
				if (
					existing &&
					thresholdDate &&
					record.modifyDate &&
					new Date(record.modifyDate) <= thresholdDate
				) {
		...
		const groupsResult =
			await this.groupsService.syncFromStudents(studentsSnapshot)
		this.logger.log(
			`Groups sync done: total=${groupsResult.total}, new=${groupsResult.created}, moves=${groupsResult.moves}, archived=${groupsResult.archived}, reactivated=${groupsResult.reactivated}`
		)
	...
	public async syncStaff(fromDate?: string): Promise<SyncResult> {
		...
		while (true) {
			...
			for (const record of records) {
				...
				await this.syncQualificationUpgrades(
					teacherId,
					record.coursesInfo
				)
			}
	...
	public async syncStudentDocuments(): Promise<number> {
		...
		this.logger.log(
			`Found ${studentsWithoutDocs.length} students without documents`
		)
		...
		for (const student of studentsWithoutDocs) {
			try {
				...
				this.logger.warn(
					`Failed to sync documents for student ${student.id}:`,
					error
				)
			}
		}
		...
		this.logger.log(
			`Student documents sync done: ${synced}/${studentsWithoutDocs.length}`
		)
		return synced
	}
	...
	public async syncStudyPrograms(): Promise<SyncResult> {
		...
		while (true) {
			const records = await this.edboService.post<
				EdboStudyProgramRecord[]
			...
			for (const record of records) {
				...
				const specialtyId =
					await this.resolveSpecialtyForStudyProgram(record)
				...
				const programData = {
					...
					qualificationName:
						record.qualificationGroupName ??
						record.studyProgramName,
					...
					qualificationGroupName:
						record.qualificationGroupName ?? null,
					universitySpecializationId:
						record.universitySpecializationId ?? null,
					specializationName: record.specializationName ?? null,
					accreditationExistsType:
						record.accreditationExistsType ?? null,
					accreditationExistsTypeName:
						record.accreditationExistsTypeName ?? null,
					edeboDateLastChange: record.dateLastChange
						? new Date(record.dateLastChange)
						: null,
				...
				const existing =
					await this.prisma.educationalProgram.findUnique({
	...
	private async resolveSpecialtyForStudyProgram(
		...
		const parsed = specialityName
			? this.parseSpecialtyCode(specialityName)
			: null
		...
		if (specialityId !== null && specialityId !== undefined) {
			...
			if (byEdeboId) {
				...
				if (parsed?.code && byEdeboId.code.startsWith('EDEBO-')) {
					const code = await this.buildUniqueSpecialtyCode(
						parsed.code,
						byEdeboId.id
					)
		...
		for (const doc of documents) {
			...
			if (doc.idPersonDocumentType === 16) {
				if (doc.documentSeries)
					result.studentTicketSeries = doc.documentSeries
				if (doc.documentNumbers)
					result.studentTicketNumbers = doc.documentNumbers
			}

			// 36 = Паспорт громадянина України
			if (doc.idPersonDocumentType === 36) {
				if (doc.documentSeries)
					result.passportSeries = doc.documentSeries
				if (doc.documentNumbers)
					result.passportNumbers = doc.documentNumbers
			}
		}
	...
	private mapStudentData(r: EdboStudentRecord) {
		return {
			...
			educationDateBegin: r.educationDateBegin
				? new Date(r.educationDateBegin)
				: null,
			educationDateEnd: r.educationDateEnd
				? new Date(r.educationDateEnd)
				: null,
			...
			personEducationPaymentTypeName:
				r.personEducationPaymentTypeName ??
				r.PersonEducationPaymentTypeName ??
				null,
	...
	private mapStaffData(r: EdboStaffRecord) {
		return {
			...
			rang: r.rang,
			// Педагогічні звання: ЄДЕБО віддає поля, раніше не переносились (звання = NULL).
			dignityIdsStr: r.dignityIdsStr,
			dignityNames: r.dignityNames,
			skillId: r.skillId,
	...
	private mapUniversityData(
		r: EdboUniversityInfo
		...
		return {
			...
			registrationDate: r.registrationDate
				...
				: null,
			universityTypeName:
				r.universityType ?? r.universityTypeName ?? null,
	 ...
	 */
	private async syncQualificationUpgrades(
		teacherId: string,
		coursesInfo: string | null
	): Promise<void> {
	...
	private resolveThreshold(
		...
		if (lastSyncDate)
			return new Date(lastSyncDate.getTime() - SYNC_OVERLAP_MS)
		return null
	}
}
