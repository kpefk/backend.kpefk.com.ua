...
export class PersonRequestList2ResponseDto {
	...
	@ApiPropertyOptional({
		description: 'Код документу на основі якого відбувається вступ'
	})
	...
	@ApiPropertyOptional({
		description: 'Чи електронна заява, що подана з кабінету вступника ПЗСО'
	})
	isEz?: boolean | null
	...
	@ApiPropertyOptional({
		description: 'Ід громадянства вступника (див. довідник PHYS_Country)'
	})
	...
	@ApiPropertyOptional({
		description:
			'Черговість в рейтинговому списку, серед вступників з однаковим конкурсним балом (черговість за мотиваційним листом)'
	})
	enrollPriority?: number | null

	@ApiPropertyOptional({
		description:
			'Допуск до конкурсу (0=тільки контракт; 1=тільки бюджет; 2=бюджет та контракт)'
	})
	enrollLevel?: number | null

	@ApiPropertyOptional({
		description: 'Тип конкурсу за яким отримано рекомендацію на бюджет'
	})
	budgetRecommendationTypeId?: number | null

	@ApiPropertyOptional({
		description: 'Тип конкурсу за яким отримано рекомендацію на контракт'
	})
	contractRecommendationTypeId?: number | null

	@ApiPropertyOptional({
		description:
			'Особа, що претендує на зарахування на навчання на підставі отриманої позитивної оцінки співбесіди (творчого конкурсу) отримала позитивну оцінку'
	})
	...
	@ApiPropertyOptional({
		description:
			'Виконано вимоги до зарахування (параллельне здобуття освіти)'
	})
	...
	@ApiPropertyOptional({
		description:
			'Освітній ступінь(рівень) за кошти державного або місцевого бюджету'
	})
	isBudgetEducation?: number | null

	@ApiPropertyOptional({
		description:
			'Чи є право безоплатно здобувати освіту за другою спеціальністю'
	})
	isAnotherBudgetAllowed?: boolean | null

	@ApiPropertyOptional({
		description:
			'Чи вступ на ту ж саму або споріднену в межах галузі знань спеціальність'
	})
	isSimiliarSpeciality?: boolean | null

	@ApiPropertyOptional({
		description:
			'Особа здобуває вищий ступінь (рівень) вищої освіти не менше одного року та виконує у повному обсязі індивідуальний навчальний план'
	})
	isGraduateOneYear?: boolean | null

	@ApiPropertyOptional({
		description: 'Чи потрібно пройти додаткові вступні випробування'
	})
	isAdditionalExam?: boolean | null

	@ApiPropertyOptional({
		description: 'Тип документу, на основі якого відбувається вступ'
	})
	documentTypeId?: number | null

	@ApiPropertyOptional({
		description: 'Серія документу, на основі якого відбувається вступ'
	})
	documentSeries?: string | null

	@ApiPropertyOptional({
		description: 'Номер документу, на основі якого відбувається вступ'
	})
	documentNumbers?: string | null

	@ApiPropertyOptional({
		description: 'Дата видачі документу, на основі якого відбувається вступ'
	})
	documentDateGet?: Date | null

	@ApiPropertyOptional({
		description: 'Ким видано документ, на основі якого відбувається вступ'
	})
	documentIssued?: string | null

	@ApiPropertyOptional({
		description:
			'Ід відзнаки документа, на основі якого відбувається вступ (див. довідник PHYS_AwardTypes)'
	})
	documentAwardTypeId?: number | null

	@ApiPropertyOptional({
		description:
			'Спосіб виготовлення (джерело походження) документу, на основі якого відбувається вступ'
	})
	...
	@ApiPropertyOptional({
		description:
			'Чи в заяві змінено бали за результатами апеляцій ЗНО, потрібно внести зміни в документацію'
	})
	needUpdateZNO?: boolean | null

	@ApiPropertyOptional({
		description:
			'Ід типу конкурсу за яким отримано рекомендацію на бюджет за результатом адресного розміщення державного замовлення'
	})
	algRecommendationTypeId?: number | null

	@ApiPropertyOptional({
		description:
			'Пріоритет заяви, яка отримала рекомендацію за результатом адресного розміщення державного замовлення'
	})
	...
	@ApiPropertyOptional({
		description: 'Ід користувача, що додав заяву (окрім електроних заяв)'
	})
	createUserId?: number | null

	@ApiPropertyOptional({
		description: 'Користувач, що додав заяву (окрім електроних заяв)'
	})
	addUserFio?: string | null

	@ApiPropertyOptional({
		description: 'Ід акту про допущену технічну помилку'
	})
	...
	@ApiPropertyOptional({
		description: 'Телефон вступника (з електроного кабінету)'
	})
	phone?: string | null

	@ApiPropertyOptional({
		description: 'Адреса e-mail вступника (з електроного кабінету)'
	})
	email?: string | null
	...
	@ApiPropertyOptional({
		description:
			'(для електроних заяв) Вступник зазначив, що претендує на додаткові бали за успішно закінчені підготовчі курси закладу у рік вступу'
	})
	isPretendFDP?: boolean | null

	@ApiPropertyOptional({
		description:
			'(для електроних заяв) Список через кому пільгових категорій (довідник ENT_PersonSpecialCategory), які вступник зазначив'
	})
	ezIsPretendList?: string | null

	@ApiPropertyOptional({
		description:
			'(для електроних заяв) Вступник зазанчив зарахувати додаткові бали до оцінки з предмету сертифікату ЗНО для випробування з цим Ід, оскільки є учасником Всеукраїнської олімпіади для професійної орієнтації закладу вищої освіти'
	})
	additionalScoreUSSPretendId?: number | null

	@ApiPropertyOptional({
		description:
			'Причина допуску лише на контракт (див. довідник ENT_ContractReason)'
	})
	...
	@ApiPropertyOptional({
		description:
			'Вступник підтвердив вибір місця навчання за цією заявою наклавши КЕП'
	})
	isSignedDecision?: boolean | null

	@ApiPropertyOptional({
		description:
			'Дата накладання КЕП вступником щодо підтвердження вибор місця навчання за цією заявою'
	})
	signDateDecision?: Date | null

	@ApiPropertyOptional({
		description:
			'Вступник підтвердив вибір місця навчання за цією заявою завантаживши сканкопію підписаної власноруч заяви'
	})
	isSignedDecisionByFile?: boolean | null

	@ApiPropertyOptional({
		description:
			'Дата завантаження сканкопії вступником щодо підтвердження вибор місця навчання за цією заявою'
	})
