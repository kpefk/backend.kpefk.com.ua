...
export class PersonRequestZNOListResponseDto {
	@ApiPropertyOptional({
		description: 'Код результату іспиту за технологією ЗНО (ЄВІ/ЄФВВ)'
	})
	znoTechnologySubjectId?: number | null

	@ApiPropertyOptional({
		description: 'Рік складання іспиту за технологією ЗНО (ЄВІ/ЄФВВ)'
	})
	subjectYear?: number | null

	@ApiPropertyOptional({
		description: 'Бал іспиту за технологією ЗНО (ЄВІ/ЄФВВ)'
	})
	...
	@ApiPropertyOptional({
		description: 'ID сертифікату в документах фіз.особи'
	})
