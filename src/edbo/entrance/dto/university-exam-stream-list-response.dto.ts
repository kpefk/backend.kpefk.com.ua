...
export class UniversityExamStreamListResponseDto {
	...
	@ApiProperty({
		description: 'Дата та час проведення випробування (потік Х)'
	})
	...
	@ApiProperty({
		description:
			'Кінцева дата реєстрації на участь у вступному випробуванні в межах потоку X'
	})
	universityExamRequestDateEnd!: Date

	@ApiProperty({
		description: 'Максимальна кількість місць для участі в межах потоку X'
	})
	universityExamRequestLimit!: number

	@ApiProperty({
		description:
			'Кількість поданих заяв на потік (скасовані не враховуються)'
	})
