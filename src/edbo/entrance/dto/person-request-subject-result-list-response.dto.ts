...
export class PersonRequestSubjectResultListResponseDto {
	@ApiPropertyOptional({
		description: 'Ід значення показника (результату) в заяві'
	})
	...
	@ApiPropertyOptional({
		description: 'Назва предмету випробування/показника'
	})
	...
	@ApiPropertyOptional({
		description: 'Ід запису про оцінку сертифікату НМТ (ЗНО) особи'
	})
	personDocumentSubjectId?: number | null

	@ApiPropertyOptional({
		description: 'Дата отримання сертифікату НМТ (ЗНО)'
	})
	...
	@ApiPropertyOptional({
		description: 'Ід запису про результат випробування ЄВІ/ЄФВВ'
	})
	znoTechnologySubjectId?: number | null

	@ApiPropertyOptional({
		description: 'Інформація про результат випробування ЄВІ/ЄФВВ'
	})
	...
	@ApiPropertyOptional({
		description:
			'Чи бал внесено сертифікатом (НМТ/ЗНО або ЄВІ/ЄФВВ, або ЄДКІ)'
	})
	isZNO?: boolean | null
	...
	@ApiPropertyOptional({
		description:
			'ДБО (Додатковий бал від участі у Всеукраїнській олімпіаді)'
	})
	additionalScore?: number | null

	@ApiPropertyOptional({
		description:
			'Б200 (Зараховано 200 балів як учаснику міжнародних олімпіад)'
	})
	extra200Score?: boolean | null

	@ApiPropertyOptional({
		description: 'Чи поле «Результат» потребує заповнення'
	})
