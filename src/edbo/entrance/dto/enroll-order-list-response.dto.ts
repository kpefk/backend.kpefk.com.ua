...
export class EnrollOrderListItemDto {
	...
	@ApiPropertyOptional({
		description: 'Кількість заяв у наказі',
		example: 25,
		nullable: true
	})
	...
	@ApiProperty({
		description: 'КЕП КПЗО: чи накладено підпис та печатку',
		example: false
	})
	isSeal!: boolean

	@ApiProperty({
		description: 'Чи для зарахування за іншими КП',
		example: false
	})
	...
	@ApiProperty({
		description: 'Чи до наказу додані заяви (неможливе видалення)',
		example: false
	})
	hasData!: boolean
}
