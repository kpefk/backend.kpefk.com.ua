...
export class PersonRequestCategoryListItemDto {
	...
	@ApiProperty({
		description: 'ID категорії (довідник ENT_PersonSpecialCategory)',
		example: 3
	})
	...
	@ApiPropertyOptional({
		description:
			'ID користувача, який створив запис (null якщо додано вступником при подачі електронної заяви)',
		example: 42,
		nullable: true
	})
	...
	@ApiPropertyOptional({
		description:
			'ПІБ користувача або позначка що додано вступником при подачі електронної заяви',
...
export type PersonRequestCategoryListResponseDto =
	PersonRequestCategoryListItemDto[]
