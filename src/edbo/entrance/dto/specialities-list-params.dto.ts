...
export class SpecialitiesListParamsDto {
	...
	@ApiPropertyOptional({
		description: 'ID відбіркової/приймальної комісії (пошук)'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description: 'Чи потребує підтвердження державним замовником (пошук)'
	})
	@IsBoolean()
	@IsOptional()
	needApprove?: boolean
	...
	@ApiPropertyOptional({
		description:
			'Результат перевірки на наявність виділених держзамовником обсягів (пошук)'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description: 'ID регіональне замовлення від (пошук)'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Номер сторінки з даними (починаючи з 0).              Якщо параметр не вказаний, він вважається рівним 0'
	})
	@IsInt()
	@IsOptional()
	pageNo?: number
	...
	@ApiPropertyOptional({
		description:
			'Розмір сторінки з даними (максимальна кількість записів, що може вернути метод)             Якщо параметр не вказаний, він вважається рівним 20'
	})
	@IsInt()
	@IsOptional()
	pageSize?: number
	...
	@ApiPropertyOptional({
		description:
			'Поле для сортування з суфіксом « DESC» для зворотнього напрямку'
	})
	@IsString()
	@IsOptional()
	sortField?: string
