...
export class PersonRequestComplexUpdateParamsDto {
	...
	@ApiPropertyOptional({
		description:
			'Категорія іноземця. Довідник значень можна отримати методом api/dictionary/get з LookupKey = CDC_ForeignType'
	})
	@IsString()
	@IsOptional()
	...
	@ApiProperty({
		description:
			'Здобуває ступінь(рівень) або вищий ступінь(рівень) не менше одного року та виконує у повному обсязі індивідуальний навчальний план'
	})
	@IsBoolean()
	isGraduateOneYear!: boolean

	@ApiProperty({
		description: 'Потрібно пройти додаткові вступні випробування'
	})
	@IsBoolean()
	...
	@ApiPropertyOptional({
		description:
			'Черговість в рейтинговому списку, серед вступників з однаковим конкурсним балом'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiProperty({
		description:
			'Освітній ступінь (рівень) за кошти державного або місцевого бюджету:                                            1 = "ніколи не здобувався"                                            2 = "вже здобутий раніше"                                            3 = "вже здобувався раніше (навчання не завершено)"'
	})
	@IsInt()
	isBudgetEducation!: number

	@ApiProperty({
		description: 'Право безоплатно здобувати освіту за другою спеціальністю'
	})
	@IsBoolean()
	...
	@ApiProperty({
		description:
			'Вступ на ту ж саму або споріднену в межах галузі знань спеціальність'
	})
	@IsBoolean()
	...
	@ApiPropertyOptional({
		description:
			'Результат співбесіди: 1 = "рекомендовано", 0 = "не рекомендовано"'
	})
	@IsBoolean()
	@IsOptional()
	...
	@ApiProperty({
		description:
			'Подано довідку про місце знаходження оригіналів документів'
	})
	@IsBoolean()
	informationOriginalDocumentLocation!: boolean

	@ApiPropertyOptional({
		description: 'Номер (шифр) особової справи, максимум 32 символи'
	})
	@IsString()
	@IsOptional()
	personalCode?: string
	...
	@ApiPropertyOptional({
		description: 'Результати вступних випробувань та комплексних показників'
	})
	@IsString()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Перелік даних, які потребують уточнення та спосіб їх подання'
	})
	@IsString()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Допущено до участі в конкурсі:2 - бюджет та контракт;1 - тільки бюджет;0 - тільки контракт'
	})
	@IsInt()
	@IsOptional()
	enrollLevel?: number
	...
	@ApiPropertyOptional({
		description: 'Причина відмови, до статусу 7 "Відмова"'
	})
	@IsString()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Код акту про допущену технічну помилку, до статусу 8 "Скасовано (ЗО)"'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Тип конкурсу за яким отримано рекомендацію на бюджет, до статусу 9 "Рекомендовано(бюджет)"'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Тип конкурсу за яким отримано рекомендацію на контракт, до статусу 12 "Рекомендовано(контракт)"'
	})
	@IsInt()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Час та місце проведення творчого конкурсу, до статусу 5 "Зареєстровано"'
	})
	@IsString()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Чи перевести статус не зважаючи на те, що до заявки прив`язано недіючий документ вступника, до статусу 14 "До наказу"'
	})
	@IsBoolean()
	@IsOptional()
	...
	@ApiPropertyOptional({
		description:
			'Чи користувач підтверджує, що ознайомлений з тим, що виконання дії викличе автоматичну ануляцію інших заяв вступника на контракт, до статусу 14 "До наказу"'
	})
	@IsBoolean()
	@IsOptional()
