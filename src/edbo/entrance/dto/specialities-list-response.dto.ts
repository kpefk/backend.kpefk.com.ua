...
export class SpecialitiesListResponseDto {
	...
	@ApiPropertyOptional({
		description: 'Назва структурного підрозділу (повна)'
	})
	...
	@ApiPropertyOptional({
		description: 'Ід редакції довідника спеціальностей'
	})
	...
	@ApiPropertyOptional({
		description: 'Чи здобуття ступеня за іншою спеціальністю'
	})
	...
	@ApiPropertyOptional({
		description: 'Максимальний обсяг державного замовлення'
	})
	maxOrder?: number | null

	@ApiPropertyOptional({
		description: 'Максимальний обсяг державного замовлення, квота 1'
	})
	maxOrderQ1?: number | null

	@ApiPropertyOptional({
		description: 'Максимальний обсяг державного замовлення, квота 2'
	})
	maxOrderQ2?: number | null

	@ApiPropertyOptional({
		description: 'Кваліфікаційний мінімум державного замовлення'
	})
	minOrder?: number | null

	@ApiPropertyOptional({
		description: 'Обсяг державного (регіонального) замовлення'
	})
	order?: number | null

	@ApiPropertyOptional({
		description: 'Загальний обсяг державного (регіонального) замовлення'
	})
	totalOrder?: number | null

	@ApiPropertyOptional({
		description: 'Обсяг державного замовлення, квота 1'
	})
	orderQ1?: number | null

	@ApiPropertyOptional({
		description: 'Обсяг державного замовлення, квота 2'
	})
	orderQ2?: number | null

	@ApiPropertyOptional({
		description: 'Обсяг державного замовлення, квота іноземців'
	})
	...
	@ApiPropertyOptional({
		description: 'Виділена частина ліцензованого обсягу'
	})
	...
	@ApiPropertyOptional({
		description:
			'Чи анульовано за результатом адресного розміщення державного (регіонального) замовлення'
	})
	isAlgCanceled?: boolean | null

	@ApiPropertyOptional({
		description: 'Чи підтверджено внесення конкурсної пропозиції'
	})
	isApplied?: boolean | null

	@ApiPropertyOptional({
		description: 'Час першого успішного внесення конкурсної пропозиції'
	})
	applyDate?: Date | null

	@ApiPropertyOptional({
		description: 'Чи потребує внесення державним (регіональним) замовником'
	})
	needApprove?: boolean | null

	@ApiPropertyOptional({
		description: 'Чи внесено обсяги державного (регіонального) замовлення'
	})
	hasOrder?: boolean | null

	@ApiPropertyOptional({
		description:
			'Чи внесено максимальні обсяги державного (регіонального) замовлення'
	})
	hasMaxOrder?: boolean | null

	@ApiPropertyOptional({
		description:
			'Чи потребує узгодження обсягів державного (регіонального) замовлення'
	})
	needChangeOrder?: boolean | null

	@ApiPropertyOptional({
		description:
			'Чи потребує узгодження максимальних обсягів державного (регіонального) замовлення'
	})
	...
	@ApiPropertyOptional({
		description:
			'Дата завершення розподілу у вступній кампанії обсягів на бюджет'
	})
	distributeBudgetOrderEndDate?: Date | null

	@ApiPropertyOptional({
		description:
			'Дата завершення розподілу у вступній кампанії обсягів на контракт'
	})
	...
	@ApiPropertyOptional({
		description:
			'Чи для спеціалізації передбачені обсяги державного (регіонального) замволення'
	})
	...
	@ApiPropertyOptional({
		description:
			'Ід результату перевірки на наявність виділених держзамовником обсягів'
	})
	orderBlockedTypeId?: number | null

	@ApiPropertyOptional({
		description:
			'Результат перевірки на наявність виділених держзамовником обсягів'
	})
	orderBlockedTypeName?: string | null

	@ApiPropertyOptional({
		description:
			'Чи вступ на цю спеціальність (спеціалізацію) та освітній ступінь має особливу підтримку'
	})
	isSpecialSupport?: boolean | null

	@ApiPropertyOptional({
		description:
			'Чи вступ на цю спеціальність (спеціалізацію) та освітній ступінь передбачає складання ЄВІ/ЄФВВ'
	})
	...
	@ApiPropertyOptional({
		description:
			'Чи для відкритої КП існує запис про відсутність широких обсягів державного замовлення для поточної вступної кампанії'
	})
	noGlobalOrder?: boolean | null

	@ApiPropertyOptional({
		description:
			'Загальна вартість за повний термін навчання розраховується автоматично'
	})
	isCountEducationAllTermPrice?: boolean | null

	@ApiPropertyOptional({
		description: 'Загальна вартість за повний термін навчання'
	})
	...
	@ApiPropertyOptional({
		description:
			'Чи акредитовані всі освітні програми конкурсної пропозиції'
	})
	...
	@ApiPropertyOptional({
		description: 'Дата та час публікації результатів адресного розміщення'
	})
