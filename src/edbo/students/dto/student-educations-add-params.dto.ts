...
8 — Доктор наук, 9 — Фаховий молодший бакалавр, 10 — Доктор мистецтва`,
	...
	@ApiProperty({
		description: 'Чи скорочений термін підготовки',
		example: false
	})
	@IsBoolean()
	isShortTerm!: boolean
	...
	@ApiProperty({
		description: 'Здобуття освіти за іншою спеціальністю',
		example: false
	})
	@IsBoolean()
	...
	@ApiPropertyOptional({
		description: 'Код попереднього навчання',
		example: 10
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'КР без отримання ПЗСО',
		example: false
	})
	@IsOptional()
	@IsBoolean()
	...
	@ApiPropertyOptional({
		description: 'Дуальна форма навчання',
		example: false
	})
	@IsOptional()
	@IsBoolean()
	...
	@ApiPropertyOptional({
		...
		example: 'Переведення з іншого ЗО'
	})
	@IsOptional()
	@IsString()
	description?: string
	...
	@ApiPropertyOptional({
		description: 'Код категорії переведення на бюджет',
		example: 1
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description:
			'Дата, до якої треба компенсувати. Ігнорується якщо oweEducationId = null',
