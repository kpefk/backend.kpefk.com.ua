...
export class StudentEducationsListForExternalRequestDto {
	...
	@ApiPropertyOptional({
		description: 'Фільтр: код картки здобувача в ЄДЕБО',
		example: 456
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Фільтр: РНОКПП здобувача',
		example: '1234567890'
	})
	@IsOptional()
	@IsString()
	...
	@ApiPropertyOptional({
		description: 'Фільтр: код закладу освіти в ЄДЕБО',
		example: 1
	})
	@IsOptional()
	@IsInt()
...
8 — Доктор наук, 9 — Фаховий молодший бакалавр, 10 — Доктор мистецтва`,
	...
	@ApiPropertyOptional({
		description: 'Фільтр: чи активний статус навчання',
		example: true
	})
	@IsOptional()
	@IsBoolean()
