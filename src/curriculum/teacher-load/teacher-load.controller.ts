...
export class TeacherLoadController {
	...
	@ApiOperation({
		...
		description:
			'Лише для керівництва. Агрегує всі робочі плани навчального року.'
	})
	...
	public getByTeacher(
		...
		return this.teacherLoadService.generateByTeacher(
			teacherId,
			academicYear
		)
	}
	...
	@ApiParam({
		name: 'workingCurriculumId',
		description: 'UUID робочого плану'
	})
	...
	public generate(
		...
		return this.subjectAssignmentsService.generate(
			workingCurriculumId,
			userId
		)
	}
	...
	@ApiResponse({
		status: 400,
		description: 'Підтверджені записи не можна редагувати'
	})
	...
	@ApiResponse({
		status: 400,
		description: 'Підтверджені записи / заборонена підгрупа'
	})
	...
	@ApiResponse({
		status: 200,
		description: 'SubjectAssignmentDto[] (перегенеровано)'
	})
	@ApiResponse({
		status: 400,
		description: 'Навантаження вже підтверджено наказом'
	})
	...
	@ApiResponse({
		status: 200,
		description: 'ConfirmSubjectAssignmentsResultDto'
	})
	@ApiResponse({
		status: 400,
		description: 'Немає DRAFT-записів / перевищення ліміту годин'
	})
	@ApiResponse({
		status: 403,
		description: 'Потрібна роль DIRECTOR або ADMINISTRATOR'
	})
	...
	@ApiResponse({
		status: 200,
		description: 'RevokeSubjectAssignmentsResultDto'
	})
	...
	@ApiQuery({
		name: 'componentTermId',
		description: 'UUID дипломного компонент-семестру'
	})
	...
	public listDiplomaSupervision(
		...
		return this.diplomaSupervisionService.listAssignableStudents(
			workingCurriculumId,
			componentTermId
		)
	}
	...
	@ApiResponse({
		status: 201,
		description: 'AssignDiplomaSupervisionResultDto'
	})
	@ApiResponse({
		status: 400,
		description: 'Цей викладач вже призначений цьому студенту'
	})
	...
	@ApiOperation({
		summary: 'Зняти призначення керівника/консультанта дипломної роботи'
	})
