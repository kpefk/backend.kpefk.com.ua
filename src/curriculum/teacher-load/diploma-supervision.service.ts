...
export class DiplomaSupervisionService {
	...
	public async listAssignableStudents(
		...
		const assignments =
			await this.prisma.diplomaSupervisionAssignment.findMany({
				where: {
					workingCurriculumId,
		...
		return students.map((s): DiplomaStudentRowDto => {
			...
			const hours = computeDiplomaSupervisionHoursPerAssignee(
				studentAssignments.length
			)
			const assignmentDtos: DiplomaAssignmentDto[] =
		...
		try {
			...
			if (
				...
				throw new BadRequestException(
					'Цей викладач вже призначений цьому студенту.'
				)
			}
			throw err
		}
	...
	public async getTeachersDiplomaHoursByAcademicYear(
		...
		if (teacherIds === undefined) {
			const distinct = await db.diplomaSupervisionAssignment.findMany({
				where: { academicYear },
				distinct: ['teacherId'],
		...
		return this.aggregateDiplomaHours(teacherIds, { academicYear }, db)
	}
	...
	private async aggregateDiplomaHours(
		...
		const teacherAssignments =
			await db.diplomaSupervisionAssignment.findMany({
		...
		const allForPairs = await db.diplomaSupervisionAssignment.findMany({
			where: {
				...scope,
		...
		for (const r of allForPairs) {
			...
			assigneeCountByPair.set(
				key,
				(assigneeCountByPair.get(key) ?? 0) + 1
			)
		}
