...
describe('computeSemesterControlHours', () => {
	...
	it('GRADED_CREDIT → та сама норма, що й CREDIT (п.14)', () => {
		expect(computeSemesterControlHours('GRADED_CREDIT', null, 2, 50)).toBe(
			4
		)
	})

	it('EXAM + ORAL → 0.33 год на студента (п.16, усно)', () => {
		expect(computeSemesterControlHours('EXAM', 'ORAL', 1, 25)).toBeCloseTo(
			8.25
		)
	})
	...
	it('EXAM + WRITTEN → 3 год на групу + 0.5 год на студента (п.16, письмово)', () => {
		...
		expect(
			computeSemesterControlHours('EXAM', 'WRITTEN', 1, 25)
		).toBeCloseTo(15.5)
		// 2 групи (потоковий облік іде по кожній групі окремо, тут перевіряємо агреговано)
		expect(
			computeSemesterControlHours('EXAM', 'WRITTEN', 2, 50)
		).toBeCloseTo(31)
	})
...
describe('computePracticeSupervisionHours', () => {
	it('EDUCATIONAL → 18 год/тиждень на групу (п.17), не залежить від studentCount', () => {
		expect(
			computePracticeSupervisionHours('PRACTICE', 'EDUCATIONAL', 2, 25)
		).toBe(36)
		expect(
			computePracticeSupervisionHours('PRACTICE', 'EDUCATIONAL', 2, 100)
		).toBe(36)
	})

	it('TECHNOLOGICAL → 1 год/студента/тиждень (п.18)', () => {
		expect(
			computePracticeSupervisionHours('PRACTICE', 'TECHNOLOGICAL', 3, 25)
		).toBe(75)
	})

	it('PRE_GRADUATION → та сама норма, що й TECHNOLOGICAL (п.18)', () => {
		expect(
			computePracticeSupervisionHours('PRACTICE', 'PRE_GRADUATION', 4, 20)
		).toBe(80)
	})

	it('componentType ≠ PRACTICE → 0 незалежно від інших параметрів', () => {
		expect(
			computePracticeSupervisionHours('DISCIPLINE', 'EDUCATIONAL', 2, 25)
		).toBe(0)
	})
	...
	it('durationWeeks = null або 0 → 0', () => {
		expect(
			computePracticeSupervisionHours('PRACTICE', 'EDUCATIONAL', null, 25)
		).toBe(0)
		expect(
			computePracticeSupervisionHours('PRACTICE', 'EDUCATIONAL', 0, 25)
		).toBe(0)
	})
})
