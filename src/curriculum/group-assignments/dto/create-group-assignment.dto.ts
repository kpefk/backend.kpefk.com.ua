...
export class CreateGroupAssignmentDto {
	...
	@ApiProperty({
		description: "Дата початку дії прив'язки",
		example: '2025-09-01'
	})
	@IsDateString(
		{},
		{ message: 'effectiveFrom має бути валідною датою (ISO 8601).' }
	)
	effectiveFrom!: string

	@ApiPropertyOptional({
		description: 'UUID користувача, що виконав призначення'
	})
	@IsOptional()
	@IsUUID('4')
	assignedBy?: string
	...
	@ApiPropertyOptional({
		description: 'Причина призначення (довільний текст)'
	})
	@IsOptional()
	@IsString()
	reason?: string
