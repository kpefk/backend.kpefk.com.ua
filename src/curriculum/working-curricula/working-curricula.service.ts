...
export class WorkingCurriculaService {
	...
	public async findAll(versionId?: string, academicYear?: string) {
		...
		const normativeCounts =
			await this.prisma.groupCurriculumAssignment.groupBy({
				by: ['versionId'],
				where: { versionId: { in: versionIds }, isActive: true },
				_count: { id: true }
			})
		const normativeCountMap = new Map(
			normativeCounts.map(r => [r.versionId, r._count.id])
		)

		// Batch: fetch which working curricula have at least one non-zero hour value
		const nonEmptyGroups =
			await this.prisma.workingCurriculumComponentTerm.groupBy({
		...
		const nonEmptyIds = new Set(
			nonEmptyGroups.map(g => g.workingCurriculumId)
		)
	...
	public async delete(id: string) {
		...
		if (wc.isApproved) {
			throw new BadRequestException(
				'Неможливо видалити затверджений робочий план.'
			)
		}
	...
	public async initializeTerms(id: string) {
		...
		const canonicalTerms =
			await this.prisma.curriculumComponentTerm.findMany({
				where: {
	...
	public async updateComponentTerm(
		termId: string,
		dto: UpdateWorkingComponentTermDto
	) {
		const term =
		...
		if (!term)
			throw new NotFoundException('Запис розподілу годин не знайдено.')
		...
		const next = {
			...
			controlWorksAuditoryCount:
				dto.controlWorksAuditoryCount ?? term.controlWorksAuditoryCount,
		...
		if (
			componentHours > 0 &&
			next.consultationHours > componentHours * consultRatio
		) {
			this.logger.warn(
		...
		return this.prisma.workingCurriculumComponentTerm.update({
			...
			data: {
				...next,
				...(dto.weeklyLectureHours !== undefined && {
					weeklyLectureHours: dto.weeklyLectureHours
				}),
				...(dto.weeklyPracticalHours !== undefined && {
					weeklyPracticalHours: dto.weeklyPracticalHours
				}),
				...
				...(dto.examFormat !== undefined && {
					examFormat: dto.examFormat
				}),
				// practiceDurationWeeks: undefined → не змінювати; null → очистити; number → встановити
				...(dto.practiceDurationWeeks !== undefined && {
					practiceDurationWeeks: dto.practiceDurationWeeks
				})
			},
			include: {
				componentTerm: {
	 ...
	 */
	public async upsertComponentTerm(
		workingCurriculumId: string,
		dto: UpsertWorkingComponentTermDto
	) {
		const wc = await this.prisma.workingCurriculum.findUnique({
			where: { id: workingCurriculumId },
			include: {
				version: {
					select: { curriculum: { select: { educationForm: true } } }
				}
			}
		})
		if (!wc)
		...
		const componentTerm =
			await this.prisma.curriculumComponentTerm.findUnique({
		...
		if (!componentTerm)
			throw new NotFoundException('Розподіл компонента не знайдено.')
	...
	public async findWorkingCurriculumForGroup(
		groupId: string,
		academicYear?: string
	) {
	...
	public async createWorkingAssignment(dto: CreateWorkingAssignmentDto) {
		...
		const workingCurriculum =
			await this.prisma.workingCurriculum.findUnique({
				where: { id: dto.workingCurriculumId }
			})
		if (!workingCurriculum)
			throw new NotFoundException('Робочий навчальний план не знайдено.')

		const normativeAssignment =
			await this.prisma.groupCurriculumAssignment.findUnique({
				where: { id: dto.assignmentId }
			})
		if (!normativeAssignment)
			throw new NotFoundException('Нормативне призначення не знайдено.')
		if (!normativeAssignment.isActive) {
			throw new BadRequestException(
				'Нормативне призначення групи є неактивним.'
			)
		}
		if (normativeAssignment.groupId !== dto.groupId) {
			throw new BadRequestException(
				'Нормативне призначення не належить до цієї групи.'
			)
		}
		...
		const existing =
			await this.prisma.groupWorkingCurriculumAssignment.findUnique({
				where: {
					groupId_academicYear: {
						groupId: dto.groupId,
