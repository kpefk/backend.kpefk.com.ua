...
export class WorkingCurriculaController {
	public constructor(
		private readonly workingCurriculaService: WorkingCurriculaService
	) {}
	...
	@ApiOperation({
		summary:
			'Отримати snapshot-призначення робочих планів для групи (аудит)',
		deprecated: true,
		description:
	...
	public findForGroup(
		...
		return this.workingCurriculaService.findWorkingCurriculumForGroup(
			groupId,
			academicYear
		)
	}
	...
	public update(
		@Param('id') id: string,
		@Body() dto: UpdateWorkingCurriculumDto
	) {
	...
	@ApiResponse({
		status: 400,
		description: "Затверджений, непорожній або з прив'язаними групами"
	})
	@ApiResponse({ status: 404 })
	@Delete(':id')
	...
	@ApiOperation({
		summary:
			"Прив'язати групу до робочого навчального плану (snapshot-запис)",
		deprecated: true,
		description:
