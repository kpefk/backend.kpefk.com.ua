...
describe('WorkingCurriculaService — delete & approve guards', () => {
	...
	describe('delete()', () => {
		it('1. Empty draft plan → deletes successfully', async () => {
			...
			prisma.workingCurriculumComponentTerm.aggregate.mockResolvedValue(
				EMPTY_AGG
			)
			prisma.workingCurriculum.delete.mockResolvedValue(makeWc())

			await expect(service.delete('wc-1')).resolves.toBeUndefined()
			expect(prisma.workingCurriculum.delete).toHaveBeenCalledWith({
				where: { id: 'wc-1' }
			})
		})
		...
		it('3. Non-empty draft plan → throws BadRequestException', async () => {
			...
			await expect(service.delete('wc-1')).rejects.toBeInstanceOf(
				BadRequestException
			)
		...
		it('5. Approved plan → throws BadRequestException without touching DB', async () => {
			prisma.workingCurriculum.findUnique.mockResolvedValue(
				makeWc({ isApproved: true })
			)
			...
			expect(
				prisma.workingCurriculumComponentTerm.aggregate
		...
		it('6. Plan with group assignments → throws BadRequestException', async () => {
			prisma.workingCurriculum.findUnique.mockResolvedValue(
				makeWc({ groupAssignmentsCount: 2 })
			)
	...
	describe('approve()', () => {
		it('2. Empty draft plan → throws BadRequestException (cannot approve)', async () => {
			...
			await expect(service.approve('wc-1')).rejects.toBeInstanceOf(
				BadRequestException
			)
		...
		it('4. Non-empty draft plan → approves successfully', async () => {
			...
			await expect(service.approve('wc-1')).resolves.toMatchObject({
				isApproved: true
			})
