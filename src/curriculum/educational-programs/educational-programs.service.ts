...
export class EducationalProgramsService {
	...
	public async findById(id: string) {
		...
		if (!program)
			throw new NotFoundException('Освітню програму не знайдено.')
		return program
	}
	...
	public async update(id: string, dto: UpdateEducationalProgramDto) {
		...
		if (dto.edeboId !== undefined && dto.edeboId !== null) {
			...
			if (conflict) {
				throw new BadRequestException(
					`ЄДЕБО ID ${dto.edeboId} вже використовується.`
				)
			}
		}
		...
		return this.prisma.educationalProgram.update({
			...
			data: {
				...
				...(dto.qualificationName !== undefined && {
					qualificationName: dto.qualificationName.trim()
				}),
				...(dto.qualificationLevel !== undefined && {
					qualificationLevel: dto.qualificationLevel?.trim() ?? null
				}),
				...(dto.edeboId !== undefined && {
					edeboId: dto.edeboId ?? null
				}),
				...(dto.approvalDate !== undefined && {
					approvalDate: dto.approvalDate
						? new Date(dto.approvalDate)
						: null
				}),
				...(dto.approvalOrderNumber !== undefined && {
