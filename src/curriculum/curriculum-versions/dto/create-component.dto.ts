...
export class CreateComponentDto {
	...
	@ApiProperty({
		description: 'Назва компонента',
		example: 'Алгоритмізація та програмування'
	})
	@IsString()
	@MinLength(2)
	@MaxLength(512)
	...
	@IsOptional()
	@IsEnum(CurriculumComponentType, {
		message: 'Невалідна категорія компонента.'
	})
	...
	@ApiPropertyOptional({
		description:
			'Загальна кількість годин (розраховується як totalEcts × 30 якщо не передано)',
		example: 360
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: "Чи обов'язковий компонент",
		default: true
	})
	@IsOptional()
	@IsBoolean()
	...
	@ApiPropertyOptional({
		enum: PracticeType,
		description: 'Вид практики (для типу PRACTICE)'
	})
	@IsOptional()
	@IsEnum(PracticeType)
	...
	@ApiPropertyOptional({
		description: 'Кількість курсових робіт',
		default: 0
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Кількість курсових проектів',
		default: 0
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Аудиторні години (загалом)',
		example: 180
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Практичні заняття (год.)',
		example: 60
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Семінарські заняття (год.)',
		example: 30
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Лабораторні роботи (год.)',
		example: 30
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Самостійна робота (год.)',
		example: 90
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Інші години (консультації тощо)',
		example: 0
	})
	@IsOptional()
	@IsInt()
	...
	@ApiPropertyOptional({
		description: 'Код вибіркової групи',
		example: 'ВК1'
	})
	@IsOptional()
	@IsString()
	...
	@ApiPropertyOptional({
		description: 'UUID батьківського компонента (для альтернатив)'
	})
	@IsOptional()
	@IsUUID('4')
