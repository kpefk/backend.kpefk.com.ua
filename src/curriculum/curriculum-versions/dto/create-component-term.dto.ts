...
export class CreateComponentTermDto {
	...
	@ApiPropertyOptional({
		description:
			'ЄКТС за семестр (необовʼязково; якщо не передано — зберігається 0)',
		example: 4.5
	})
	@IsOptional()
	@IsNumber()
	...
	@ApiPropertyOptional({
		description: 'Наявність курсової роботи',
		default: false
	})
	@IsOptional()
	@IsBoolean()
	...
	@ApiPropertyOptional({
		description: 'Наявність курсового проекту',
		default: false
	})
	@IsOptional()
	@IsBoolean()
	...
	@ApiPropertyOptional({
		description:
			'Кількість підгруп для практ./лаб. (1 або 2). Наказ МОН №686 п.5–6.',
		example: 2,
		minimum: 1,
		maximum: 2
	})
	...
	@Min(1)
	@Max(2, {
		message: 'Наказ МОН №686 п.5–6 дозволяє поділ лише на 2 підгрупи.'
	})
	...
	@ApiPropertyOptional({
		description:
			"Підстава для поділу на підгрупи (обов'язкова при subgroupCount = 2)",
