...
export class CurriculumVersionsController {
	...
	@ApiOperation({
		summary: 'Отримати версію навчального плану з повною структурою'
	})
	...
	@ApiResponse({
		status: 400,
		description: 'Версія опублікована, застаріла або вже використовується'
	})
	@ApiResponse({ status: 404, description: 'Не знайдено' })
	...
	@ApiOperation({
		summary: 'Дублювати структуру версії у новий чернетковий план'
	})
	@ApiParam({ name: 'sourceVersionId', description: 'UUID вихідної версії' })
	@ApiParam({
		name: 'curriculumId',
		description: 'UUID навчального плану для нової версії'
	})
	...
	@ApiOperation({
		summary:
			'Додати відображення компонента в додатковий розділ (інтеграційна проекція)'
	})
	@ApiParam({
		name: 'versionId',
		description: 'UUID версії навчального плану'
	})
	@ApiResponse({ status: 201 })
	@ApiResponse({
		status: 400,
		description:
			'Той самий розділ, крос-версійна проекція, версія опублікована або дублікат'
	})
	...
	@ApiOperation({
		summary: 'Додати/оновити тиждень в графіку навчального процесу (upsert)'
	})
	@ApiParam({ name: 'versionId', description: 'UUID версії' })
