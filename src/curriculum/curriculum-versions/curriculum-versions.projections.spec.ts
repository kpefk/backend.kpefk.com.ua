...
function makeDraftVersion(
	overrides: Partial<{ id: string; versionNumber: number }> = {}
) {
	return {
...
describe('CurriculumVersionsService — projection invariants', () => {
	...
	beforeEach(async () => {
		...
		service = module.get<CurriculumVersionsService>(
			CurriculumVersionsService
		)
	})
	...
	describe('published version immutability', () => {
		...
		it('deleteProjection throws when canonical component belongs to a published version', async () => {
			...
			prisma.curriculumComponentDisplayInSection.findUnique.mockResolvedValue(
				projection
			)

			await expect(service.deleteProjection('proj-1')).rejects.toThrow(
				BadRequestException
			)
			expect(
				prisma.curriculumComponentDisplayInSection.delete
	...
	describe('duplicateFrom — projection ID remapping', () => {
		it('re-creates projections with new component/section IDs', async () => {
			...
			prisma.curriculumVersion.findFirst.mockResolvedValue({
				versionNumber: 1
			})
			...
			const txMock = {
				...
				curriculumComponent: {
					create: jest.fn().mockResolvedValue(newComp1)
				},
				curriculumComponentTerm: { create: jest.fn() },
				curriculumComponentDisplayInSection: {
					findMany: jest.fn().mockResolvedValue([
			...
			expect(
				txMock.curriculumComponentDisplayInSection.create
