...
function assertDraft(
	version: Pick<CurriculumVersion, 'isPublished' | 'versionNumber'>
): void {
...
export class CurriculumVersionsService {
	...
	public async findById(id: string) {
		const version = await this.prisma.curriculumVersion.findUnique({
			...
			include: {
				...
				_count: {
					select: { groupAssignments: true, workingCurricula: true }
				}
			}
		})
		...
		const projections =
			await this.prisma.curriculumComponentDisplayInSection.findMany({
	...
	public async publish(id: string) {
		...
		const sectionCount = await this.prisma.curriculumSection.count({
			where: { versionId: id }
		})
		if (sectionCount === 0) {
			throw new BadRequestException(
	...
	public async deleteVersion(versionId: string): Promise<void> {
		...
		await this.prisma.$transaction(async tx => {
			...
			const componentIds = sections.flatMap(s =>
	...
	public async duplicateFrom(sourceVersionId: string, curriculumId: string) {
		...
		return this.prisma.$transaction(async tx => {
					...
					const newComponent = await tx.curriculumComponent.create({
						data: {
							...
							electiveBlockId: component.electiveBlockId
								? (blockIdMap.get(component.electiveBlockId) ??
									null)
								: null,
							code: component.code,
							name: component.name,
			...
			const sourceProjections =
				await tx.curriculumComponentDisplayInSection.findMany({
	...
	public async updateSection(sectionId: string, dto: UpdateSectionDto) {
		const section = await this.prisma.curriculumSection.findUnique({
			where: { id: sectionId },
			include: {
				version: { select: { isPublished: true, versionNumber: true } }
			}
		})
	...
	public async deleteSection(sectionId: string) {
		...
		return this.prisma.$transaction([
			...
			this.prisma.curriculumComponent.deleteMany({
				where: { sectionId }
			}),
	...
	public async createElectiveBlock(
		sectionId: string,
		dto: CreateElectiveBlockDto
	) {
		const section = await this.prisma.curriculumSection.findUnique({
			where: { id: sectionId },
			include: {
		...
		if (dto.maxSelections < dto.minSelections) {
			throw new BadRequestException(
				'maxSelections не може бути менше за minSelections.'
			)
		}
	...
	public async deleteElectiveBlock(id: string) {
		const block = await this.prisma.electiveBlock.findUnique({
			...
			include: {
				section: {
					include: {
						version: {
							select: { isPublished: true, versionNumber: true }
						}
					}
				},
		...
		if (!block)
			throw new NotFoundException(
				'Блок вибіркових компонентів не знайдено.'
			)
	...
	public async createComponent(sectionId: string, dto: CreateComponentDto) {
		...
		if (dto.electiveBlockId) {
			...
			if (!block || block.sectionId !== sectionId) {
				throw new BadRequestException(
					'Блок вибіркових компонентів не належить до цього розділу.'
				)
			}
		}
			...
			data: {
				...
				...(dto.totalEcts !== undefined && {
					...
					totalHours:
						Number(dto.totalEcts) > 0
							? Math.round(Number(dto.totalEcts) * 30)
							: dto.totalHours !== undefined
								? dto.totalHours
								: Number(component.totalHours)
				}),
				...
				...(dto.courseWorkCount !== undefined && {
					courseWorkCount: dto.courseWorkCount
				}),
				...(dto.courseProjectCount !== undefined && {
					courseProjectCount: dto.courseProjectCount
				}),
				...(dto.notes !== undefined && { notes: dto.notes }),
				...(dto.electiveBlockId !== undefined && {
					electiveBlockId: dto.electiveBlockId ?? null
				}),
				// Hours breakdown
				...(dto.auditoryHours !== undefined && {
					auditoryHours: dto.auditoryHours ?? null
				}),
				...
				...(dto.labHours !== undefined && {
					labHours: dto.labHours ?? null
				}),
				...(dto.selfStudyHours !== undefined && {
					selfStudyHours: dto.selfStudyHours ?? null
				}),
				...(dto.otherHours !== undefined && {
					otherHours: dto.otherHours ?? null
				}),
				...(dto.znoPreparationHours !== undefined && {
					znoPreparationHours: dto.znoPreparationHours ?? null
				}),
				// Elective group
				...
				...(dto.parentComponentId !== undefined && {
					parentComponentId: dto.parentComponentId ?? null
				})
			},
			include: { terms: true }
	...
	public async deleteComponent(componentId: string) {
		...
		return this.prisma.$transaction([
			...
			this.prisma.curriculumComponent.delete({
				where: { id: componentId }
			})
		])
	}
	...
	public async createProjection(
		versionId: string,
		dto: CreateComponentProjectionDto
	) {
		...
		if (!targetSection)
			throw new NotFoundException('Цільовий розділ не знайдено.')
	...
	public async deleteProjection(projectionId: string): Promise<void> {
		const projection =
			await this.prisma.curriculumComponentDisplayInSection.findUnique({
		...
		await this.prisma.curriculumComponentDisplayInSection.delete({
			where: { id: projectionId }
		})
	}
	...
	public async createComponentTerm(
		componentId: string,
		dto: CreateComponentTermDto
	) {
		const component = await this.prisma.curriculumComponent.findUnique({
			where: { id: componentId },
			include: {
	...
	public async updateComponentTerm(
		termId: string,
		dto: UpdateComponentTermDto
	) {
		...
		const newJustification =
			dto.subgroupJustification !== undefined
		...
		return this.prisma.curriculumComponentTerm.update({
			...
			data: {
				...
				...(dto.controlForm !== undefined && {
					controlForm: dto.controlForm
				}),
				...(dto.hasCourseWork !== undefined && {
					hasCourseWork: dto.hasCourseWork
				}),
				...(dto.hasCourseProject !== undefined && {
					hasCourseProject: dto.hasCourseProject
				}),
				...(dto.subgroupCount !== undefined && {
	...
	public async deleteComponentTerm(termId: string) {
		...
		return this.prisma.curriculumComponentTerm.delete({
			where: { id: termId }
		})
	}
	...
	public async createTimeBudgetEntry(
		versionId: string,
		dto: CreateTimeBudgetEntryDto
	) {
		const version = await this.prisma.curriculumVersion.findUnique({
			where: { id: versionId }
		})
	 ...
	 */
	public async upsertCalendarEntry(
		versionId: string,
	...
	public async deleteCalendarEntry(entryId: string) {
		...
		return this.prisma.academicCalendarEntry.delete({
			where: { id: entryId }
		})
	}
}
