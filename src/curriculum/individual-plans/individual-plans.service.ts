	...
	public async create(dto: CreateIndividualPlanDto, approvedBy: string) {
		...
		if (existing) {
			throw new BadRequestException(
				'Студент вже має індивідуальний навчальний план.'
			)
		}
		...
		if (!assignment)
			throw new NotFoundException(
				'Призначення групи до плану не знайдено.'
			)
		...
		const confirmedSelections =
			await this.prisma.studentElectiveSelection.findMany({
				...
				include: {
					...
					season: {
						select: { block: { select: { semesterNumber: true } } }
					}
				}
			})
			...
			data: {
				...
				items: {
					create: confirmedSelections.map(sel => ({
						...
						deviationType:
							IndividualPlanDeviationType.ELECTIVE_SELECTION,
						notes:
