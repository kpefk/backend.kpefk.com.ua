...
function mapSectionType(
	...
	if (part === 'SECONDARY') {
		if (n.includes('базові компоненти'))
			return CurriculumSectionType.BASIC_OPP
		if (n.includes('вибірково-обов'))
			return CurriculumSectionType.ELECTIVE_OPP
		if (n.includes('факультатив'))
			return CurriculumSectionType.OPTIONAL_COURSES
		return CurriculumSectionType.SECONDARY_EDUCATION
	}
	if (n.includes('загальні компетентн'))
		return CurriculumSectionType.GENERAL_COMPETENCY
	if (n.includes('за вибором') || n.includes('вибіркова'))
		return CurriculumSectionType.ELECTIVE
...
function mapComponentType(name: string): ComponentType {
	...
	if (n.includes('захист кваліфікаційної'))
		return ComponentType.QUALIFICATION_WORK_DEFENSE
	if (
		n.includes('кваліфікаційн') &&
		(n.includes('іспит') || n.includes('екзамен'))
	)
...
function mapPracticeType(name: string): PracticeType | null {
	...
	if (n.includes('технологічна') || n.includes('виробнич'))
		return PracticeType.TECHNOLOGICAL
...
function mapComponentKind(
	...
	if (componentType === ComponentType.PRACTICE)
		return CurriculumComponentType.PRACTICE
	if (
...
export class CurriculumImportService {
	...
	public preview(file: Express.Multer.File): ParsedCurriculum {
		return parseCurriculumWorkbook(
			file.buffer,
	...
	public async commit(dto: ImportCommitDto) {
		...
		if (!program)
			throw new NotFoundException('Освітню програму (ОПП) не знайдено.')
		...
		return this.prisma.$transaction(async tx => {
			...
			for (let si = 0; si < parsed.sections.length; si++) {
				...
				const realComponents = sec.components.filter(
					c => !c.isElectivePlaceholder
				)
				...
				for (let ci = 0; ci < realComponents.length; ci++) {
					...
					if (
						sec.part === 'PROFESSIONAL' &&
						comp.code &&
						integrationTargets.has(comp.code)
					) {
						...
						const effective: ParsedComponent = delivery
							? {
									...comp,
									totalHours:
										delivery.totalHours ?? comp.totalHours,
						...
						const id = await this.createComponent(tx, {
							...
							blockByGroup,
							noteOverride:
								'Інтегрований освітній компонент — викладається у складі ЗСО'
						})
	...
	private async createComponent(
		...
		const notes =
			args.noteOverride ??
			(comp.integratedWithCode
				? `Інтегровано з ${comp.integratedWithCode}`
				: null)
			...
			data: {
				...
				terms: {
					create: comp.terms.map(t => {
						const termEcts =
							ects > 0 && termHoursSum > 0
								? round2((ects * t.hours) / termHoursSum)
								: 0
						return {
							...
							hasCourseWork: comp.courseWorkSemesters.includes(
								t.semesterNumber
							)
						}
					})
