...
function detectColumns(rows: Matrix): ColumnMap {
	...
	for (let r = 0; r < HEADER_SCAN; r++) {
		const count = (header[r] ?? []).filter(c =>
			lower(c).includes('семестр')
		).length
		if (count >= 2) {
			semesterMarkerRow = r
			break
	...
	if (semesterMarkerRow >= 0) {
		...
		markerCells.forEach((colIdx, i) => {
			...
			const semNo =
				explicit && explicit >= 1 && explicit <= 12 ? explicit : i + 1
	...
	const map: ColumnMap = {
		code: findCol(
			header,
			s => s.includes('код') || s === '№ з/п' || s.includes('з/п')
		),
		name: findCol(
			header,
			s => s.includes('назва') || s.includes('найменування')
		),
		...
		totalHours: findCol(
			header,
			s => s.includes('загальний обсяг') || s === 'годин'
		),
		...
		practical: findCol(
			header,
			s => s.includes('практичн') || s.includes('проактичн')
		),
...
function parsePlanSheet(
	...
	for (const [label, idx] of required) {
		if (idx < 0)
			warnings.push(
				`Не знайдено колонку «${label}» у шапці аркуша «План».`
			)
	}
	if (cols.semesterCols.size === 0) {
		warnings.push(
			'Не знайдено семестрових колонок (маркер «семестр тижнів»).'
		)
	}
	...
	const weeksRow =
		rows[cols.headerEndRow - 1] ?? rows[cols.headerEndRow] ?? []
	for (const [semNo, colIdx] of [...cols.semesterCols].sort(
		(a, b) => a[0] - b[0]
	)) {
	...
	for (let r = cols.headerEndRow + 1; r < rows.length; r++) {
		...
		const totalHours =
			cols.totalHours >= 0 ? toNumber(row[cols.totalHours]) : null
		...
		const component: ParsedComponent = {
			...
			electiveGroupCode: isElective
				? code
				: isPlaceholder
					? lastElectiveGroupCode
					: null,
			...
			ects: cols.ects >= 0 ? toNumber(row[cols.ects]) : null,
			auditoryHours:
				cols.auditory >= 0 ? toNumber(row[cols.auditory]) : null,
			lectureHours:
				cols.lecture >= 0 ? toNumber(row[cols.lecture]) : null,
			practicalHours:
				cols.practical >= 0 ? toNumber(row[cols.practical]) : null,
			seminarHours:
				cols.seminar >= 0 ? toNumber(row[cols.seminar]) : null,
			labHours: cols.lab >= 0 ? toNumber(row[cols.lab]) : null,
			selfStudyHours:
				cols.selfStudy >= 0 ? toNumber(row[cols.selfStudy]) : null,
		...
		if (!total && !toNumber(row[cTheory >= 0 ? cTheory : 0])) {
			if (course.toLowerCase() === 'всього') {
				budget.push({
					...
					sessionWeeks:
						cSession >= 0 ? toNumber(row[cSession]) : null,
					practiceWeeks:
						cPractice >= 0 ? toNumber(row[cPractice]) : null,
					attestationWeeks:
						cAttest >= 0 ? toNumber(row[cAttest]) : null,
					holidayWeeks:
						cHoliday >= 0 ? toNumber(row[cHoliday]) : null,
					totalWeeks: total
				})
			}
			continue
...
export function parseCurriculumWorkbook(
	...
	if (titleName && wb.Sheets[titleName]) {
		const titleRows = XLSX.utils.sheet_to_json<string[]>(
			wb.Sheets[titleName],
			{
				header: 1,
				raw: false,
