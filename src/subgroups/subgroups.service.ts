...
export class SubgroupsService {
		...
		const [assignments, activeCount] = await Promise.all([
			this.prisma.studentSubgroup.groupBy({
				...
				where: {
					...
					curriculumComponentTermId: { in: termIds }
				},
				_count: { _all: true }
			}),
	...
	public async getSplit(
		...
		const byStudent = new Map(
			memberships.map(m => [m.studentId, m.subgroupNumber])
		)
	...
	public async autoSplit(
		...
		const assignments = students.map((s, i) => ({
			studentId: s.id,
			subgroupNumber: Math.min(
				dto.subgroupCount,
				Math.floor(i / perGroup) + 1
			)
		}))
	...
	private async resolveSubject(
		...
		if (!wt) {
			throw new NotFoundException(
				'Дисципліну не знайдено в РНП цієї групи.'
			)
		}
		return {
