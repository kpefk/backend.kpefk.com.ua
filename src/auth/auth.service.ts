...
export class AuthService {
	...
	public async login(
		...
		if (method === 'EMAIL') {
			...
			if (!dto.code) {
				return {
					message: 'Введіть 6-значний код з програми Authenticator.'
				}
			}
			if (!user.totpSecret) {
				throw new UnauthorizedException(
					'TOTP secret not configured. Contact administrator.'
				)
			}
			const plainSecret = this.totpService.decrypt(user.totpSecret)
			if (!this.totpService.verify(dto.code, plainSecret)) {
				throw new UnauthorizedException(
					'Невірний код Authenticator. Перевірте застосунок та спробуйте знову.'
				)
			}
		}
	...
	public async register_student(
		...
		const userId = await this.prisma.$transaction(async tx => {
			...
			const studentData = this.mapEdboStudentData(
				edeboResult.student,
				dto,
				edeboResult.documents
			)
			...
			const studentRecord = await tx.student.findFirst({
				where: {
					personId: edeboResult.student.personId,
					universityId:
						edeboResult.student.universityId ||
						this.configService.getOrThrow<number>('EDEBO_CODE')
				}
			})
	...
	private async findStudentInEdebo(dto: RegisterStudentDto) {
		...
		if (!dto.no_rnokpp && dto.rnokpp) {
			...
			const edeboResult = await this.edboService.findStudentByRnokpp(
				dto.rnokpp,
				universityId
			)
			if (edeboResult) {
	...
	private mapEdboStudentData(
		...
		const ticketDoc = edeboDocuments.find(
			d => d.idPersonDocumentType === 16
		) // Студентський квиток
		const passportDoc = edeboDocuments.find(
			d => d.idPersonDocumentType === 36
		) // Паспорт

		return {
			personId: edeboStudent.personId,
			universityId:
				edeboStudent.universityId ||
				this.configService.getOrThrow<number>('EDEBO_CODE'),
			...
			dateBegin: edeboStudent.dateBegin
				? new Date(edeboStudent.dateBegin)
				: null,
			dateEnd: edeboStudent.dateEnd
				? new Date(edeboStudent.dateEnd)
				: null,
			historyTypeId: edeboStudent.historyTypeId,
			personFIO: edeboStudent.personFIO,
			birthday: edeboStudent.birthday
				? new Date(edeboStudent.birthday)
				: null,
			personSexName: edeboStudent.personSexName,
			licenseYear: edeboStudent.licenseYear,
			educationDateBegin: edeboStudent.educationDateBegin
				? new Date(edeboStudent.educationDateBegin)
				: null,
			educationDateEnd: edeboStudent.educationDateEnd
				? new Date(edeboStudent.educationDateEnd)
				: null,
			...
			personEducationPaymentTypeName:
				edeboStudent.personEducationPaymentTypeName,
			...
			modifyDate: edeboStudent.modifyDate
				? new Date(edeboStudent.modifyDate)
				: null,
			...
			rnokpp:
				rnokppDoc?.documentNumbers ||
				(dto.no_rnokpp ? '' : (dto.rnokpp ?? '')),
			studentTicketSeries:
				ticketDoc?.documentSeries ||
				(dto.no_student_ticket ? '' : (dto.serial_ticket ?? '')),
			studentTicketNumbers:
				ticketDoc?.documentNumbers ||
				(dto.no_student_ticket ? '' : (dto.number_ticket ?? '')),
			passportSeries:
				passportDoc?.documentSeries || (dto.serial_passport ?? ''),
			passportNumbers:
				passportDoc?.documentNumbers || (dto.number_passport ?? ''),
	...
	public async getProfile(
		...
		const studentUser =
			await this.userService.findByIdWithStudentProfile(userId)
