...
export class SurveysController {
  ...
  @ApiOperation({ summary: 'Пройти опитування (один раз; анонімно, якщо задано кампанією)' })
  @Authorization(UserRole.STUDENT)
  @Post(':id/submit')
  @HttpCode(HttpStatus.OK)
  public async submit(
  ...
  public create(@Body() dto: CreateSurveyDto, @Authorized('id') userId: string) {
  ...
  @Put(':id/questions')
  @HttpCode(HttpStatus.OK)
  public setQuestions(@Param('id') id: string, @Body() dto: SetSurveyQuestionsDto) {
    return this.service.setQuestions(id, dto)
  }

  @ApiOperation({ summary: 'Змінити статус: DRAFT→OPEN, OPEN→CLOSED' })
  @Patch(':id/status')
  @HttpCode(HttpStatus.OK)
  public setStatus(@Param('id') id: string, @Body() dto: SetSurveyStatusDto) {
  ...
  @ApiOperation({ summary: 'Результати: агрегати по питаннях + response rate' })
  @Get(':id/results')
  @HttpCode(HttpStatus.OK)
