...
export class ElectiveSeasonsController {
	...
	@ApiOperation({
		summary: '(Адмін) Оновити метадані кампанії (педрада, наказ, дедлайни)'
	})
	@ApiParam({ name: 'id' })
	@Authorization('DEPUTY_DIRECTOR', 'DIRECTOR', 'ADMINISTRATOR')
	...
	@ApiOperation({
		summary: '(Адмін) Bulk-перехід статусу кампанії та всіх її сезонів'
	})
	@ApiParam({ name: 'id' })
	...
	@ApiOperation({
		summary: "(Адмін) Згенерувати block-сезони кампанії за прив'язками груп"
	})
	@ApiParam({ name: 'id' })
	@Authorization('DEPUTY_DIRECTOR', 'DIRECTOR', 'ADMINISTRATOR')
	...
	@ApiOperation({
		summary: '(Адмін) Зведення прогресу вибору по групах кампанії'
	})
	@ApiParam({ name: 'id' })
	@Authorization(
		'HEAD_OF_DEPARTMENT',
	...
	@ApiOperation({
		summary:
			'(Адмін) Зафіксувати підсумок вибору групи: кворум/наказ + пропагація в РНП'
	})
	...
	public confirmGroupSelection(
		...
		return this.service.confirmGroupSelection(
			seasonId,
			groupId,
			dto,
			adminId
	...
	@ApiOperation({
		summary: "Мої блоки ВК (через прив'язку групи до навчального плану)"
	})
