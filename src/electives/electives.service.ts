...
export class ElectivesService {
	...
	public async updateSeasonStatus(
		seasonId: string,
	...
	public async deleteSeason(seasonId: string) {
		...
		if (season.catalogStatus !== CatalogStatus.DRAFT) {
			throw new BadRequestException(
				'Видалення дозволено лише для сезонів зі статусом DRAFT.'
			)
		}
		if (season._count.selections > 0) {
			throw new BadRequestException(
				'Не можна видалити сезон, у якому є вибори студентів.'
			)
		}

		await this.prisma.electiveBlockSeason.delete({
			where: { id: seasonId }
		})
	...
	public async addOffering(seasonId: string, dto: CreateOfferingDto) {
		...
		if (!component)
			throw new NotFoundException('CurriculumComponent не знайдено.')
		...
		const existing = await this.prisma.electiveOffering.findUnique({
			where: {
				seasonId_componentId: { seasonId, componentId: dto.componentId }
			}
		})
		if (existing)
			throw new BadRequestException(
				'Цей варіант вже є в каталозі сезону.'
			)
	...
	public async removeOffering(offeringId: string) {
		...
		if (offering.season.catalogStatus !== CatalogStatus.DRAFT) {
			throw new BadRequestException(
				'Видалення варіанту дозволено лише для DRAFT сезонів.'
			)
		}
	...
	public async studentSelect(studentId: string, dto: StudentSelectDto) {
		const season = await this.prisma.electiveBlockSeason.findUnique({
			...
			include: {
				block: { select: { minSelections: true, maxSelections: true } }
			}
		})
		if (!season) throw new NotFoundException('Сезон не знайдено.')
		...
		if (!offering)
			throw new NotFoundException(
				'Цей варіант недоступний у поточному сезоні.'
			)
	...
	public async studentCancelSelection(
		selectionId: string,
		studentId: string
	) {
		...
		if (selection.season.catalogStatus !== CatalogStatus.OPEN) {
			throw new BadRequestException(
				'Скасування можливе лише під час відкритого вікна (OPEN).'
			)
		}
	...
	public async confirmSelectionsV2(
		academicYear: string,
		adminUserId: string
	) {
		...
		this.logger.log(
			`[confirmSelectionsV2] Confirmed ${confirmed.count} selections for ${academicYear}`
		)
		return { confirmed: confirmed.count }
	}

	public async getGroupSelectionStatsV2(
		groupId: string,
		academicYear: string
	) {
		...
		const selections = await this.prisma.studentElectiveSelection.findMany({
			...
			include: {
				...
				season: {
					include: { block: { select: { semesterNumber: true } } }
				}
			}
		})
	...
	public async autoAssignBulk(
		...
		if (students.length === 0) {
			throw new NotFoundException(
				`Групу ${dto.groupId} не знайдено або вона порожня.`
			)
		}
		...
		if (unassigned.length === 0) {
			this.logger.log(
				`[autoAssignBulk] Всі студенти групи ${dto.groupId} вже мають вибір у сезоні ${seasonId}`
			)
			return {
				assigned: 0,
				componentId: resolvedComponentId,
		...
		const cancelledRecords =
			await this.prisma.studentElectiveSelection.findMany({
				where: {
					seasonId,
		...
		const cancelledByStudentId = new Map(
			cancelledRecords.map(r => [r.studentId, r.id])
		)
	...
	public async getStudentsWithoutSelectionV2(
		groupId: string,
		seasonId: string
	) {
		const season = await this.prisma.electiveBlockSeason.findUnique({
	...
	public async updateComponent(id: string, dto: UpdateElectiveComponentDto) {
		...
		if (dto.curriculumTermId) {
			...
			if (!term)
				throw new NotFoundException(
					'Терм навчального плану не знайдено.'
				)
		...
		return this.prisma.electiveComponent.update({
			...
			data: {
				...
				syllabusUrl:
					dto.syllabusUrl !== undefined
						? dto.syllabusUrl
						: component.syllabusUrl,
				isHigherEd:
					dto.isHigherEd !== undefined
						? dto.isHigherEd
						: component.isHigherEd,
				curriculumTermId:
					dto.curriculumTermId !== undefined
						? dto.curriculumTermId
						: component.curriculumTermId
			},
			include: CURRICULUM_TERM_INCLUDE
		})
	}
	...
	public async deleteComponent(id: string) {
		...
		if (component.catalogStatus !== CatalogStatus.DRAFT) {
			throw new BadRequestException(
				'Видалення дозволено лише для ВК зі статусом DRAFT.'
			)
		}
		if (component._count.registrations > 0) {
			throw new BadRequestException(
				'Не можна видалити ВК, на який є реєстрації студентів.'
			)
		}
	...
	public async cloneCatalog(dto: CloneElectiveCatalogDto) {
		if (dto.sourceYear === dto.targetYear) {
			throw new BadRequestException(
				'Рік-джерело і рік-ціль не можуть співпадати.'
			)
		}
		...
		if (source.length === 0) {
			throw new NotFoundException(
				`Каталог ВК для ${dto.sourceYear} не знайдено.`
			)
		}
		...
		this.logger.log(
			`[cloneCatalog] Cloned ${created.length} ВК from ${dto.sourceYear} → ${dto.targetYear}`
		)
		return { cloned: created.length, targetYear: dto.targetYear }
	}

	/** @deprecated */
	public async updateCatalogStatus(
		electiveId: string,
	...
	public async selectElective(studentId: string, dto: SelectElectiveDto) {
		const elective = await this.prisma.electiveComponent.findUnique({
			where: { id: dto.electiveId }
		})
	...
	public async cancelSelection(registrationId: string, studentId: string) {
		...
		if (reg.elective.catalogStatus !== CatalogStatus.OPEN) {
			throw new BadRequestException(
				'Скасування можливе лише під час відкритого вікна вибору (OPEN).'
			)
		}
	...
	public async getGroupSelectionStats(groupId: string, academicYear: string) {
		...
		for (const reg of registrations) {
			...
			if (entry) {
				...
				byElective.set(reg.electiveId, {
					elective: reg.elective,
					count: 1
				})
			}
		}
	...
	public async confirmSelections(academicYear: string, adminUserId: string) {
		...
		this.logger.log(
			`[confirmSelections] Confirmed ${confirmed.count} pending registrations for ${academicYear}`
		)
		return { confirmed: confirmed.count, autoAssigned: 0 }
	}

	/** @deprecated */
	public async getGroupEnrollmentList(
		groupId: string,
		electiveId: string,
		academicYear: string
	) {
	...
	/** @deprecated */
	public async getStudentsWithoutSelection(
		groupId: string,
		semester: number,
		academicYear: string
	) {
	...
	public async getAppendix3Data(seasonId: string, groupId: string) {
		...
		return {
			...
			semester: season.block.semesterNumber,
			academicYear:
				season.electiveSeason?.academicYear ?? season.academicYear,
			group: group.name,
			rows: students.map((s, i) => {
				...
				return {
					...
					methodLabel:
						sel?.method === SelectionMethod.VOLUNTARY
							? 'Заява'
			...
			voluntaryCount: selections.filter(
				s => s.method === SelectionMethod.VOLUNTARY
			).length,
			assignedCount: selections.filter(
				s => s.method === SelectionMethod.ASSIGNED
			).length
		}
	}
	...
	public async validateElectiveThreshold(versionId: string) {
		...
		for (const section of version.sections) {
			if (
				section.sectionType === 'ELECTIVE' ||
				section.sectionType === 'ELECTIVE_OPP'
			) {
			...
			for (const block of section.electiveBlocks) {
				if (block.options.length > 0) {
					const maxEcts = Math.max(
						...block.options.map(o => Number(o.totalEcts))
					)
