...
export class ElectivesController {
	...
	@ApiOperation({
		summary: '(Адмін) Призначити ВК студенту (нова архітектура)'
	})
	@Authorization(
		'HEAD_OF_DEPARTMENT',
		'DEPUTY_DIRECTOR',
	...
	@HttpCode(HttpStatus.CREATED)
	public adminAssignV2(
		@Req() req: Request & { user: User },
		@Body() dto: AdminAssignV2Dto
	) {
	...
	@ApiOperation({
		summary: '(Адмін) Масове автопризначення наказом (нова архітектура)'
	})
	@ApiParam({ name: 'seasonId' })
	@Authorization(
		'HEAD_OF_DEPARTMENT',
	...
	@ApiOperation({
		summary: '(Адмін) Масове підтвердження (нова архітектура)'
	})
	@ApiQuery({ name: 'academicYear', required: true })
	@Authorization('DIRECTOR', 'ADMINISTRATOR')
	...
	@ApiOperation({
		summary: '(Адмін) Статистика вибору по групі (нова архітектура)'
	})
	...
	@ApiOperation({
		summary: 'Дані для друку Додатку 2 (заява студента, §3.4)'
	})
	...
	@ApiOperation({
		summary: 'Дані для друку Додатку 3 (список зарахованих, §3.9)'
	})
	@ApiQuery({ name: 'seasonId', required: true })
	@ApiQuery({ name: 'groupId', required: true })
	@Authorization(
	...
	@ApiOperation({
		summary: 'Перевірка порогу 10% ВК (ст.54 п.17 Закону 2745-VIII)'
	})
	...
	public updateComponent(
		@Param('id') id: string,
		@Body() dto: UpdateElectiveComponentDto
	) {
	...
	/** @deprecated */
	@ApiOperation({
		summary: "[deprecated] Терми навчального плану для прив'язки"
	})
	...
	public getEnrollmentList(
		...
		return this.service.getGroupEnrollmentList(
			groupId,
			electiveId,
			academicYear
		)
	...
	public getUnselected(
		...
		return this.service.getStudentsWithoutSelection(
			groupId,
			+semester,
			academicYear
		)
