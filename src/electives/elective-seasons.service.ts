 ...
 */
export function semestersForAcademicYear(
	entryYear: number,
	academicYear: string
): number[] {
...
export class ElectiveSeasonsService {
			...
			include: {
				blockSeasons: {
					include: {
						...
						_count: {
							select: { offerings: true, selections: true }
						}
					},
	...
	public async createCampaign(dto: CreateCampaignDto) {
		...
		if (existing) {
			throw new BadRequestException(
				`Кампанія для ${dto.academicYear} вже існує.`
			)
		}
		...
		return this.prisma.electiveSeason.create({
			data: {
				...
				selectionDeadline: dto.selectionDeadline
					? new Date(dto.selectionDeadline)
					: undefined,
				lateDeadline: dto.lateDeadline
					? new Date(dto.lateDeadline)
					: undefined,
				notes: dto.notes
			},
			include: CAMPAIGN_INCLUDE
	...
	public async updateCampaign(id: string, dto: UpdateCampaignDto) {
		...
		return this.prisma.electiveSeason.update({
			...
			data: {
				...
				...(dto.lateDeadline !== undefined && {
					lateDeadline: new Date(dto.lateDeadline)
				}),
				...
				...(dto.pedagogicalCouncilProtocolNumber !== undefined && {
					pedagogicalCouncilProtocolNumber:
						dto.pedagogicalCouncilProtocolNumber
				}),
	...
	public async generateBlockSeasons(id: string) {
		...
		for (const a of assignments) {
			...
			const versionBlockSemesters = blockSemestersByVersion.get(
				a.versionId
			)
		...
		const blocks =
			semestersByVersion.size === 0
				...
				: await this.prisma.electiveBlock.findMany({
						where: {
							OR: [...semestersByVersion.entries()].map(
								([versionId, semesters]) => ({
									section: { versionId },
	...
	public async getMyBlocks(studentId: string, academicYear: string) {
		...
		const assignment =
			await this.prisma.groupCurriculumAssignment.findFirst({
				where: {
				...
				select: {
					versionId: true,
					curriculum: {
						select: { entryYear: true, studyDurationMonths: true }
					}
				},
		...
		const semesters = semestersForAcademicYear(
			...
			s => s <= Math.ceil(assignment.curriculum.studyDurationMonths / 6)
		)
	...
	public async confirmGroupSelection(
		...
		if (!assignment) {
			throw new BadRequestException(
				"Група не має активної прив'язки до навчального плану."
			)
		}
		...
		if (voluntary.length > 0) {
			...
			quorumReached =
				top._count.componentId / groupSize >= QUORUM_THRESHOLD
			if (!componentId) {
				if (!quorumReached) {
					const percent = Math.round(
						(top._count.componentId / groupSize) * 100
					)
					throw new BadRequestException(
		...
		if (!offering) {
			throw new NotFoundException(
				'Обраний компонент відсутній у каталозі цього сезону.'
			)
		}
		...
		const withoutSelection = students.filter(
		...
		const cancelledByStudent = new Map(
			cancelled.map(c => [c.studentId, c.id])
		)
		const toCreate = withoutSelection.filter(
			s => !cancelledByStudent.has(s.id)
		)
		const toRevive = withoutSelection.filter(s =>
			cancelledByStudent.has(s.id)
		)
			...
			...(winnersPending.length > 0
				? [
						this.prisma.studentElectiveSelection.updateMany({
							where: {
								id: { in: winnersPending.map(s => s.id) }
							},
							data: {
	...
	private async propagateToWorkingCurriculum(
		...
		if (!wc.semesterNumbers.includes(semesterNumber))
			return { status: 'SEMESTER_NOT_COVERED' }

		const term = await this.prisma.curriculumComponentTerm.findUnique({
			where: {
				componentId_semesterNumber: { componentId, semesterNumber }
			}
		})
	...
	public async getCampaignProgress(id: string) {
		...
		const outcomeKey = (seasonId: string, assignmentId: string) =>
			`${seasonId}:${assignmentId}`
		const outcomeMap = new Map(
			groupOutcomes.map(o => [
				outcomeKey(o.seasonId ?? '', o.assignmentId),
				o
			])
		)
		...
		for (const a of assignments) {
			...
			const relevantSeasons = campaign.blockSeasons.filter(
				s =>
					s.block.section.versionId === a.versionId &&
					semesters.has(s.block.semesterNumber)
			)
			...
			for (const s of relevantSeasons) {
				...
				const voluntaryCount = grouped.reduce(
					(sum, g) => sum + g._count.componentId,
					0
				)
				let topComponent: {
					id: string
				...
				if (grouped.length > 0) {
					const top = grouped[0]
					const comp =
						await this.prisma.curriculumComponent.findUnique({
				...
				rows.push({
					...
					quorumReached:
						!!topComponent &&
						groupSize > 0 &&
						topComponent.count / groupSize >= QUORUM_THRESHOLD,
					outcome: outcome
						? {
