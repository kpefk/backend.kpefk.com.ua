...
export class GroupLeaderService {
	...
	public async getStudent(
		...
		if (!student)
			throw new NotFoundException('Студента не знайдено в цій групі.')
		...
		return {
			...
			personEducationPaymentTypeName:
				student.personEducationPaymentTypeName,
	...
	public async upsertParentInfo(
		...
		this.logger.log(
			`[ParentInfo] userId=${userId} updated studentId=${studentId}`
		)
		return info
	}
	...
	public async getParentInfoAdmin(
		studentId: string,
		userId: string,
		req: Request
	) {
	...
	public async upsertParentInfoAdmin(
		...
		this.logger.log(
			`[ParentInfo/admin] userId=${userId} updated studentId=${studentId}`
		)
		return info
	}
	...
	public async exportGroupExcel(
		...
		const rows = group.students.map((s, i) => ({
			'№': i + 1,
			ПІБ: s.personFIO,
			'Дата народження': s.birthday
				? s.birthday.toLocaleDateString('uk-UA')
				: '',
	...
	private async auditRead(
		userId: string,
		action:
			'READ_PARENT_INFO' | 'VIEW_GROUP_PARENT_INFO' | 'EXPORT_GROUP_DATA',
		targetId: string,
