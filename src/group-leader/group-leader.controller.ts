...
export class GroupLeaderController {
	...
	@ApiOperation({
		summary: 'Список студентів групи з батьківськими контактами'
	})
	...
	public upsertParentInfo(
		...
		return this.service.upsertParentInfo(
			groupId,
			studentId,
			dto,
			req.user.id,
	...
	public async exportExcel(
		...
		const buf = await this.service.exportGroupExcel(
			groupId,
			req.user.id,
			req
		)
		...
		res.setHeader(
			'Content-Disposition',
			`attachment; filename="group-${groupId}.xlsx"`
		)
		res.end(buf)
	}
	...
	public upsertParentInfoAdmin(
		...
		return this.service.upsertParentInfoAdmin(
			studentId,
			dto,
			req.user.id,
			req
