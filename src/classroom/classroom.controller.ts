...
export class ClassroomController {
	...
	public async proxyPhoto(
		...
		const { stream, mimeType } =
			await this.googleDriveService.streamFile(googleFileId)
	...
	@ApiResponse({
		status: 200,
		description: 'Оновлений кабінет із новим фото'
	})
	@ApiResponse({ status: 400, description: 'Невірна капча' })
	@ApiResponse({ status: 403, description: 'Доступ заборонено' })
	@ApiResponse({ status: 404, description: 'Кабінет не знайдено' })
	...
	@ApiResponse({
		status: 200,
		description: 'Оновлений кабінет без видаленого фото'
	})
	@ApiResponse({ status: 403, description: 'Доступ заборонено' })
	@ApiResponse({ status: 404, description: 'Кабінет не знайдено' })
	@Authorization(UserRole.ADMINISTRATOR, UserRole.TEACHER)
	...
	@ApiResponse({
		status: 200,
		description: 'Оновлений кабінет із новим порядком фото'
	})
	@ApiResponse({ status: 403, description: 'Доступ заборонено' })
	@ApiResponse({ status: 404, description: 'Кабінет не знайдено' })
	@Authorization(UserRole.ADMINISTRATOR, UserRole.TEACHER)
