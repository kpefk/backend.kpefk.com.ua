...
export class ClassroomService {
	...
	public async delete(id: string) {
		...
		const fileIds = [
			...
			...(classroom.passportGoogleFileId
				? [classroom.passportGoogleFileId]
				: [])
		]
		...
		await Promise.all(
			fileIds.map(fileId =>
				this.googleDriveService.deleteFile(fileId).catch(err => {
					this.logger.error(
						`Failed to delete Drive file ${fileId}:`,
						err
					)
				})
			)
	...
	public async assertKnownPassportFileId(
		googleFileId: string
	): Promise<void> {
		const exists = await this.prismaService.classroom.findFirst({
	...
	public async uploadPhoto(
		...
		const { url, googleFileId } = await this.googleDriveService.uploadPhoto(
			file,
			classroom.number
		)
	...
	public async uploadPassport(
		...
		if (classroom.passportGoogleFileId) {
			await this.googleDriveService
				.deleteFile(classroom.passportGoogleFileId)
				.catch(err => {
					this.logger.error(
						`Failed to delete old passport ${classroom.passportGoogleFileId}:`,
						err
					)
				})
		}

		const { url, googleFileId } =
			await this.googleDriveService.uploadPassport(file, classroom.number)
