...
export class AttendanceScheduleService {
	...
	public async resolveForTeacher(
		...
		const { weekday, academicYear, term, dateParity, dayStart, dayEnd } =
			ctx
		...
		for (const entry of ownEntries) {
			const lesson = this.materialize(
				entry,
				ownSubs.get(entry.id),
				term,
				dateParity
			)
	...
	private materialize(
		...
		if (!semesterMatchesTerm(entry.schedule.semesterNumber, term))
			return null
		if (!entryVisibleOnDate(entry.weekParity, dateParity)) return null
		if (sub?.type === 'CANCELLED') return null

		const isTeacherChange =
			sub?.type === 'TEACHER_CHANGE' && !!sub.replacementTeacherId
		...
		const slotNumber =
			sub?.type === 'MOVED' && sub.newSlotNumber
				? sub.newSlotNumber
				: entry.slotNumber
	...
	private sortBySlot(lessons: ResolvedLesson[]): ResolvedLesson[] {
		return lessons.sort(
			(a, b) =>
				...
				a.groupName.localeCompare(b.groupName, 'uk')
		)
	}
}
