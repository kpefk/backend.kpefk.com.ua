...
export class JournalService {
	...
	private async gatherData(
		...
		const recordMap = new Map<
			string,
			{ status: AttendanceStatus; grade: number | null }
		>()
		for (const r of records) {
			recordMap.set(`${r.lessonSessionId}:${r.studentId}`, {
				status: r.status,
				grade: r.grade
			})
		}
		...
		const teacherName = sessions[0]?.teacher
			? [
					sessions[0].teacher.lastName,
					sessions[0].teacher.firstName,
					sessions[0].teacher.middleName
				]
					.filter(Boolean)
					.join(' ')
			: ''
	...
	private buildMatrixTable(
		...
		const dataRows = studentRows.map(
			(s, idx) =>
				new TableRow({
					children: [
						this.matrixCell(
