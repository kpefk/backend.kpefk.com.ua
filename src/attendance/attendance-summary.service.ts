...
export class AttendanceSummaryService {
	...
	public async getAttendanceSummary(
		...
		const summaryMap = new Map<
			string,
			{ attended: number; late: number; absent: number; grades: number[] }
		>()
			...
			return {
				...
				attendancePercent:
					totalLessons > 0
						...
						: 0,
				averageGrade:
					entry && entry.grades.length > 0
						? Math.round(
								(entry.grades.reduce((a, b) => a + b, 0) /
									entry.grades.length) *
									10
							) / 10
						: null
	...
	public async getStudentSemesterSummary(
		...
		const recordBySession = new Map(
			records.map(r => [r.lessonSessionId, r])
		)
		...
		const disciplines: StudentDisciplineSummaryDto[] = Array.from(
			byTerm.entries()
		).map(([termId, e]) => {
			...
			return {
				...
				attendancePercent:
					e.total > 0
						? Math.round((presentCount / e.total) * 100)
						: 0,
				averageGrade:
					e.grades.length > 0
						? Math.round(
								(e.grades.reduce((a, b) => a + b, 0) /
									e.grades.length) *
									10
							) / 10
						: null
