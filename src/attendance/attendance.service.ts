...
import {
	...
	type ResolvedLesson
} from './attendance-schedule.service'
import { academicYearOf, parseDateOnly, termOf } from './attendance.calendar'
import { isElevated, TEACHER_BACKFILL_DAYS } from './attendance.constants'
import type {
	AttendanceRowDto,
	LessonSessionDto,
	LessonSlotDto,
...
export class AttendanceService {
	...
	public async listLessons(
		...
		const sessionByKey = new Map(
			sessions.map(s => [
				this.sessionKey(s.groupId, s.slotNumber, s.subgroupNumber),
				s
			])
		)
	...
	public async openSession(
		...
		const lesson = dayLessons.find(
			l => l.scheduleEntryId === dto.scheduleEntryId
		)
		if (!lesson) {
	...
	public async carryOver(
		...
		const prevStatus = new Map(
			prev.records.map(r => [r.studentId, r.status])
		)
	...
	private async resolveLessons(
		...
		if (actor.role === UserRole.TEACHER) {
			...
			return teacherId
				? this.scheduleResolver.resolveForTeacher(teacherId, date)
				: []
		}
		// Керівництво.
		if (query.groupId)
			return this.scheduleResolver.resolveForGroup(query.groupId, date)
		if (query.teacherId)
			return this.scheduleResolver.resolveForTeacher(
				query.teacherId,
				date
			)
		...
		if (ownTeacherId)
			return this.scheduleResolver.resolveForTeacher(ownTeacherId, date)
	...
	private async loadSessionOrThrow(id: string) {
		...
		if (!session || session.deletedAt)
			throw new NotFoundException('Сторінку журналу не знайдено.')
		return session
	}
	...
	private async loadAndMap(
		...
		const session = await this.prisma.lessonSession.findUnique({
			...
			include: {
				...
				curriculumComponentTerm: {
					select: { component: { select: { code: true } } }
				},
				records: true
			}
		})
		...
		const canEdit =
			(await this.canEditLesson(session.teacherId, actor)) &&
			this.isWritable(session.date, actor.role)
	...
	private async assertCanViewLesson(
		lessonTeacherId: string,
		actor: Actor
	): Promise<void> {
	...
	private sameSemester(a: Date, b: Date): boolean {
		return (
			academicYearOf(a) === academicYearOf(b) && termOf(a) === termOf(b)
		)
	}
