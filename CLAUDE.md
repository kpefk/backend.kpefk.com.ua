...
> Файли: `src/setup/`, `prisma/seed.ts`

- **Першого адміністратора створює користувач з клієнта**, а не seed.
  `POST /setup/administrator` + `GET /setup/status`. У репозиторії немає і не має
  з'являтися жодного захардкоженого пароля.
- **Маршрути публічні свідомо** — на порожній базі нікому авторизуватись. Закриває їх
  стан системи: щойно є хоч один користувач → 409.
- **Критерій — `user.count() === 0`, а не «немає адміністратора».** Інакше видалення
  останнього адміна знову відкрило б маршрут на базі з даними здобувачів.
- **Транзакція `Serializable`** — два паралельні запити на порожню базу не мають
  створити двох адміністраторів. `P2034` і `P2002` мапляться на 409, не на 500.
- **`prisma db seed` НЕ створює користувачів** — лише довідники (спеціальності з
  `normativeEcts` зі стандартів, ОПП). Додаси туди користувача — зламаєш первинне
  налаштування, бо маршрут закриється до того, як його відкриють.
...
- **Три тіри:** `development` / `test` (staging) / `production`. Обирає `NODE_ENV`.
- **Поведінку задає матриця `POLICIES`, а не розсипані перевірки.** Немає `isDev()` —
  він був бінарним і трактував `test` як production. Потрібна нова відмінність між
  тірами → додай прапорець у `EnvironmentPolicy`; `Record<AppEnvironment, …>` не дасть
  забути жоден тір.
- **Записи в ЄДЕБО дозволені ТІЛЬКИ в production.** Блок стоїть в `EdboService.post()` —
  єдиній точці, через яку йдуть усі виклики реєстру. Перемикача немає свідомо: ЄДЕБО
  не має пісочниці, всі тіри ходять у бойовий реєстр тими самими обліковими даними.
  **Не додавай змінну-обхід.** Новий write-ендпоінт → допиши його останній сегмент
  у `EdboService.WRITE_SEGMENTS` і в `environment.spec.ts`.
- **Env-файли:** `.env.<tier>` перекриває базовий `.env`. Завантаження — при імпорті
  `environment.ts`, тобто до `ConfigModule.forRoot()`. Прив'язка через
  `config/env.validation.ts` — не прибирай звідти `import './environment'`,
  інакше ConfigModule прочитає незаповнений `process.env`.
- **`prisma.config.ts` теж імпортує `environment`** — без цього `NODE_ENV=test
  prisma migrate deploy` пішов би мігрувати dev-базу.
- **Комітимо лише `*.example`.** `.env`, `.env.development`, `.env.test`,
  `.env.production` — у `.gitignore`.
- **`SESSION_FOLDER` має бути різний у кожному тірі** — інакше сесії змішуються в Redis.
...
- **Усі зміни схеми — тільки через міграцію:**
  ...
  ```
- **Ніколи `prisma db push` на спільних базах.** Саме через це домен дипломів
  (5 таблиць, 4 енуми) жив лише в dev/production, а тір, розгорнутий з нуля,
  піднімався без нього. Закрито міграцією `20260918100000_diploma_domain_baseline`.
  CI-job `Migrations` тепер ловить такий розрив: накочує історію на чисту базу
  і порівняє результат зі `schema.prisma`.
- **Міграція, що чіпає об'єкти, які вже є в dev/prod, має бути ідемпотентною**
  (`IF NOT EXISTS`, перевірки `pg_type` / `pg_constraint`) — інакше `migrate deploy`
  впаде на наявній базі.
